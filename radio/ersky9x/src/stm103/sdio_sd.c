...
//void wherec( uint8_t chr ) ;
//void refresh( void ) ;
//void lcd_outhex4c(uint8_t x, uint8_t y, uint16_t val) ;

//uint32_t Cresp1Status ;

extern uint16_t DtStart ;
extern uint16_t DtEnd ;
...
/** 
  ...
  */
#define SDIO_TRANSFER_CLK_DIV            ((uint8_t)0x01)		// Was 0 
	...
	while (!(SDIO->STA &(SDIO_FLAG_RXOVERR | SDIO_FLAG_DCRCFAIL | SDIO_FLAG_DTIMEOUT | SDIO_FLAG_DBCKEND | SDIO_FLAG_STBITERR)))
  {
    if (SDIO_GetFlagStatus(SDIO_FLAG_RXFIFOHF) != RESET)
    {
      ...
      tempbuff += 8;
		...
		//lcd_outhex4c(0, 32, SDIO->STA ) ;
		//lcd_outhex4c(30, 32, SDIO_GetFlagStatus(SDIO_FLAG_RXFIFOHF) ) ;
//		refresh() ;
		if ( g_tmr10ms - i > 5 )
		{
...
//  SD_ProcessDMAIRQ();
  ...
  MX_FATFS_Init();
   ...
   == sd_state = HAL_SD_Init(&hsd);
	...
	err = app_SDCard_link(&fatfs, &mynewdiskPath);
	
	err = app_SDCard_mount(&fatfs);
