...
//					}
        ...
        for( x = -15, y = 2 ; y < 9 ; x += 5, y += 1 )
        {
          ...
          len = ((calibratedStick[z]+RESX)/((RESX*2)/BAR_HEIGHT))+1 ;  // calculate once per loop
          V_BAR(SCREEN_WIDTH/2+12+x,SCREEN_HEIGHT-8, len ) ;
				}
#endif
    }
#endif // ARUNI
...
void menuTextHelp(uint8_t event)
{
	...
	else if ( ( event == EVT_KEY_BREAK(KEY_MENU) ) || ( event == EVT_KEY_BREAK(BTN_RE) ) )
	{
		TextResult = 1 ;
  	s_editMode = 0 ;
		killEvents(event) ;
...
#if defined(PCBSKY) || defined(PCB9XT)
 else if ( sub < 8+page2SizeExtra) // sub>=7
#else
 else if ( sub < 8+page2SizeExtra) // sub>=7
#endif
 {
...
//	}
...
#if defined(PCBSKY) || defined(PCB9XT)
 else if ( sub < 13+page2SizeExtra) // sub>=8
#else
 else if ( sub < 13+page2SizeExtra) // sub>=8
#endif
	{
#if defined(PCBSKY) || defined(PCB9XT)
		uint8_t subN = 8+page2SizeExtra ;
#else
		uint8_t subN = 8+page2SizeExtra ;
#endif
		y = FH ;
	...
	else if ( sub <= 19 + page2SizeExtra )
	{
#if defined(PCBSKY) || defined(PCB9XT)
		uint8_t subN = 13+page2SizeExtra ;
#else
		uint8_t subN = 13+page2SizeExtra ;
#endif
		y = FH ;
...
{
	static MState2 mstate2 ;
	mstate2.check_columns(event, 16-1+1 ) ;	
...
#define NUM_SCALE_DESTS		12
				if( attr )
				{
					CHECK_INCDEC_H_MODELVAR( epscaler->dest, 0, NUM_SCALE_DESTS ) ;
				}
				lcd_putsAttIdx( 11*FW, y, XPSTR("\005-----BaseMAmps mAh  VoltsFuel RBSV Cus1 Cus2 Cus3 Cus4 Cus5 Cus6"), epscaler->dest, attr ) ;
			break ;
		}
	}
...
{
	uint32_t rows = mode ? NUM_GLOBAL_VOICE_ALARMS : NUM_VOICE_ALARMS + NUM_EXTRA_VOICE_ALARMS + 1 ;
...
//			uint8_t z = k - ( NUM_VOICE_ALARMS + NUM_EXTRA_VOICE_ALARMS ) ;
		...
		{
	    if(k == NUM_VOICE_ALARMS + NUM_EXTRA_VOICE_ALARMS ) break;
  ...
  if( k == NUM_VOICE_ALARMS + NUM_EXTRA_VOICE_ALARMS )
	{
    //last line available - add the golbal voice alarms line
...
uint16_t extendedValueEdit( int16_t value, uint8_t extValue, uint8_t attr, uint8_t y, uint8_t event )
{
	if ( (value <= -126) || (value >= 126) )
	{
		// A gvar ;
	...
	value = gvarMenuItem( FW*16, y, value, -125, 125, attr | GVAR_250, event ) ;
	if ( value > 500 )
	{
		value -= 501 - 126 ;
		if ( value > 128 )
...
//lcd_outhex4( 90, FH, NumSw2 ) ;
        ...
        case 1:
          ...
          lcd_puts_P(  2*FW,y,PSTR(STR_2WEIGHT));
					uint16_t result = extendedValueEdit( md2->weight, md2->extWeight, attr, y, event ) ;
...
//					}
        ...
        case 2:
          ...
          lcd_puts_P( FW, y, PSTR(STR_OFFSET) ) ;
					uint16_t result = extendedValueEdit( md2->sOffset, md2->extOffset, attr, y, event ) ;
...
//						pmd->weight = gvarMenuItem( 7*FW+FW/2, y, pmd->weight, -125, 125, attr | GVAR_250, 0 ) ;
					 ...
					 if ( g_eeGeneral.altMixMenu )
					 {
						if (sub == current)
						{
							...
							lcd_puts_P( 13*FW, FH, XPSTR("Off") ) ;
    					lcd_outdez( 20*FW, FH, pmd->sOffset ) ;

							lcd_puts_P( 13*FW, 2*FH, XPSTR("Sw") ) ;
	            putsDrSwitches( 16*FW, 2*FH , pmd->swtch, 0 ) ;

							lcd_puts_P( 13*FW, 6*FH, XPSTR("D   :") ) ;
						  lcd_outdezAtt( FW*17, 6*FH, pmd->delayUp, PREC1 ) ;
						  lcd_outdezAtt( FW*20, 6*FH, pmd->delayDown, PREC1 ) ;

							lcd_puts_P( 13*FW, 7*FH, XPSTR("S   :") ) ;
						  lcd_outdezAtt( FW*17, 7*FH, pmd->speedDown, PREC1 ) ;
						  lcd_outdezAtt( FW*20, 7*FH, pmd->speedUp, PREC1 ) ;
...
#else // PCBX7
#if defined(PCBX12D)    
			putHwSwitchName((2+j*(15+4))*FW-2, y, i, 0 ) ;
#else
...
#endif // PCBX7
#ifdef PCBX12D
			lcd_putc((4+j*(15+4))*FW-2, y, PSTR(HW_SWITCHARROW_STR)[pos] ) ;
#else
		...
		else
		{
...
#endif
			{
...
#endif	// REV9E
				{
					...
					k += switchPosition(k) ;
#ifdef PCBX12D
					putSwitchName((2+j*(15+4))*FW-2, y, k-HSW_OFFSET, 0) ;
#else
...
#else // PCBX7
#ifdef PCBX12D
			putSwitchName((2+j*(15+4))*FW-2, y, i+1-m, getSwitch00(i+2-m) ? INVERS : 0 ) ;
#else
...
#define TL 27
  	//                        LH LV RV RH
#if defined(PCBX12D)
  	static uint8_t x[4]    = {128*1/4+2, 4, 128+24-4, 128*3/4-2+24};
#else
...
#ifdef PCBX12D
		displayTimer( x+18*FW-1, FH*2, 0, DBLSIZE|CONDENSED ) ;
#else
...
#ifndef ARUNI
			lcd_rect( 6*FW+1, 2*FH-1, 6*FW+2, 9 ) ;   // put a bounding box
#endif
		}
		else
		{
            ...
            else if ( tview == 0x30 )
            {
                if (frskyUsrStreaming)
                {
                  ...
                  value = FrskyHubData[FR_ALT_BARO] + AltOffset ;
									...
									lcd_puts_P(10*FW, 4*FH, XPSTR("Amax"));
									putsTelemetryChannel( 18*FW, 3*FH, TEL_ITEM_BALT, FrskyHubMaxMin.hubMax[FR_ALT_BARO] + AltOffset, DBLSIZE | LEFT | CONDENSED, 0 ) ;
...
// Mavlink - home direction and distance on graphic?								
  ...
  {
#ifdef PCBX12D
		displayTimer( 30+5*FW+12, FH*5, 1, DBLSIZE ) ;
...
//#define BT_TYPE_HC05		1
      ...
      TITLE( XPSTR("BlueTooth") ) ;
			...
			g_eeGeneral.BtType = checkIndexed( y, XPSTR(FWx16"\002""\005HC-06HC-05CC-41"), g_eeGeneral.BtType, (sub==subN) ) ;
			y += FH ;
			subN += 1 ;
...
void menuProcBt(uint8_t event)
{
  MENU(XPSTR("BT"), menuTabStat, e_bluetooth, 6, {0/*, 0*/});
