...
const int8_t TelemIndex[] = { FR_A1_COPY, FR_A2_COPY,	FR_RXRSI_COPY, FR_TXRSI_COPY,	TIMER1, TIMER2,	// 0-5
														  ...
														  FR_FUEL, FR_A1_MAH, FR_A2_MAH, FR_CELL_MIN,                           // 12-15
															...
															FR_CUST1,FR_CUST2,FR_CUST3,FR_CUST4,FR_CUST5,FR_CUST6,FMODE } ;							// 68-74
...
1, 1, 1, 0, 0, 0, 2, 2, 2, 2, 2, 2, 2, 0, 0, 2, 0, 2, 0, 2, 2, 2, 2, 2, 2, 0, 0, 0, 0, 0, 0, 0, 2, 1, 2, 2, 2, 3, 3, 3, 3, 3, 3, 3, 3, 0, 0, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2, 2,2,2,2,2,2,2,0  } ;
...
// Preset from Rom data
//	index = 0 ;
//	text = MultiRomData ;

//	while ( *text )
//	{
//		Multiprotocols[index] = 0x4000 | ( MultiRomData - text ) ;
//		while ( *text++ )
//		{
//			// move past first '\0'
//		}
...
//				nextEntry |= count << 10 ;
	...
	*text = '\0' ;
	...
	{
		if ( Multiprotocols[index] & 0xC000 )
		{
	...
	for ( index = 1 ; index < 64 ; index += 1 )
	{
		if ( index < NUM_MULTI_PROTOCOLS )
		{
			if ( ( Multiprotocols[index] & 0xC000 ) == 0 )
			{
				MultiMapping[offset++] = index - 1 ;
			}
		}
  ...
  if ( attr & GVAR_250 )
	{
		...
		{
	    lcd_outdezAtt(x, y, value, attr ) ;
  	  if (invers) value = checkIncDec16( value, -250, 250, EE_MODEL ) ;
		}
		if (invers)
		{
			if ( Tevent == EVT_TOGGLE_GVAR )
...
{
//#ifdef TRIMS_SCALED
//	lcd_putsAtt( 19*FW, 7*FH, XPSTR("->"), BLINK ) ;
//#else
...
void telltale( uint8_t centrex, int8_t xval, int8_t yval )
{
  ...
  DO_CROSS( centrex, BOX_CENTERY,3 ) ;
	DO_RSQUARE( centrex +( xval/((2*RESX/16)/BOX_LIMIT)), BOX_CENTERY-( yval/((2*RESX/16)/BOX_LIMIT)), MARKER_WIDTH ) ;
}
...
//					}
        ...
        for( x = -15, y = 2 ; y < 9 ; x += 5, y += 1 )
        {
          ...
          len = ((calibratedStick[z]+RESX)/((RESX*2)/BAR_HEIGHT))+1 ;  // calculate once per loop
          V_BAR(SCREEN_WIDTH/2+12+x + X12SMALL_OFFSET,SCREEN_HEIGHT-8, len ) ;
				}
#endif
    }
#endif // ARUNI
...
uint8_t unmapMixSource( uint8_t index, uint8_t *switchSource )
{
	...
	if ( index < EXTRA_POTS_START )
	{
		if ( index > PPM_BASE + NUM_PPM )
		{
			if ( index <= PPM_BASE + NUM_PPM + NUM_EXTRA_PPM )
			{
				index += EXTRA_PPM_BASE - (PPM_BASE + NUM_PPM) + (num_mix_switches-1) ;
			}
			else
			{
				index -= NUM_EXTRA_PPM ;
				if ( index > CHOUT_BASE + NUM_SKYCHNOUT )
				{
					if ( index <= CHOUT_BASE + NUM_SKYCHNOUT + EXTRA_SKYCHANNELS )
					{
						index += EXTRA_CHANS_BASE - (CHOUT_BASE + NUM_SKYCHNOUT) + (num_mix_switches-1) ;
					}
					else
					{
						index -= EXTRA_SKYCHANNELS ;
	...
	if ( index >= MIX_3POS )
	{
		if ( index > (uint32_t)MIX_3POS + (num_mix_switches-1) )
		{
			if ( index < EXTRA_POTS_START )
			{
				index -= (num_mix_switches-1) ;
...
void menuTextHelp(uint8_t event)
{
...
#endif
#ifdef PCBX7
		max = NUM_SKYXCHNRAW+1+MAX_GVARS+1+NUM_SCALERS+8+NumExtraPots + (num_mix_switches-1) - 1 + EXTRA_SKYCHANNELS - 1 ;
#else // PCBX7
		max = NUM_SKYXCHNRAW+1+MAX_GVARS+1+NUM_SCALERS+8+NumExtraPots + (num_mix_switches-1) - 1 + EXTRA_SKYCHANNELS ;
#endif // PCBX7
		if ( TextOption )
		{
			TextOption = 1 ;
  ...
  if(sub<1) index = 0 ;
	...
	for ( i = 0 ; i < width * 6 ; i += 1 )
	{
		...
		if ( type == 0 )
		{
			putsAttIdxTelemItems( x, y, index, att | TSSI_TEXT ) ;
		}
		else if ( type == 1 )
		{
...
#define NUM_SCALE_DESTS		13
				...
				lcd_putsAttIdx( 11*FW, y, XPSTR("\005-----BaseMAmps mAh  VoltsFuel RBSV Cus1 Cus2 Cus3 Cus4 Cus5 Cus6 Aspd "), epscaler->dest, attr ) ;
			break ;
		}
	}
...
//	uint8_t x = TITLEP( PSTR(STR_EDIT_MIX));
	...
	if ( event == EVT_ENTRY_UP )
	{
		...
		if ( TextResult )
		{
			...
			{
				cs.v1 = unmapPots( TextIndex ) ;
			}
			else
			{
				cs.v2 = unmapPots( TextIndex ) ;
			}
	 	  eeDirty(EE_MODEL) ;
		}
	}
	...
	lcd_puts_Pleft( FH, XPSTR("Function\037v1\037v2\037AND\037Delay") ) ;

  if(cstate == CS_VOFS)
  {
		lcd_puts_Pleft( 3*FH, XPSTR("val") ) ;
		putsChnRaw( 9*FW, 2*FH, cs.v1, sub == 1 ? blink : 0 ) ;
    if ( (cs.v1 > CHOUT_BASE+NUM_SKYCHNOUT) && ( cs.v1 < EXTRA_POTS_START ) )
		{
			...
			putsTelemetryChannel( 9*FW, 3*FH, cs.v1-CHOUT_BASE-NUM_SKYCHNOUT-1, value, sub==2 ? blink : 0, TELEM_NOTIME_UNIT | TELEM_UNIT| TELEM_CONSTANT ) ;
		}
    else
  ...
  {
    putsChnRaw( 9*FW, 2*FH, cs.v1, sub==1 ? blink : 0 ) ;
    putsChnRaw( 9*FW, 3*FH, cs.v2, sub==2 ? blink : 0 ) ;
  }
	    ...
	    switch(i)
			{
      	    ...
      	    CHECK_INCDEC_H_MODELVAR_0( pvad->mute, 1 ) ;
  	  		...
  	  		lcd_puts_Pleft( y, XPSTR("FileType") ) ;
					lcd_putsAttIdx( 14*FW, y, XPSTR("\006------  NameNumberEffect"),pvad->fnameType,attr ) ;
...
#ifdef PCBX7
						CHECK_INCDEC_H_MODELVAR( x, 1,NUM_SKYXCHNRAW+1+MAX_GVARS+1+NUM_SCALERS+8+NumExtraPots + (num_mix_switches-1) + EXTRA_SKYCHANNELS-1 ) ;
#else // PCBX7
						CHECK_INCDEC_H_MODELVAR( x, 1,NUM_SKYXCHNRAW+1+MAX_GVARS+1+NUM_SCALERS+8+NumExtraPots + (num_mix_switches-1) + EXTRA_SKYCHANNELS ) ;
#endif // PCBX7
						if ( md2->srcRaw > 4 )
						{
							if ( ( event == EVT_KEY_LONG(KEY_MENU) ) || ( event == EVT_KEY_BREAK(BTN_RE) ) )
...
//lcd_outhex4( 90, 0, NumSw1 ) ;
//lcd_outhex4( 90, FH, NumSw2 ) ;
...
//						pmd->weight = gvarMenuItem( 7*FW+FW/2, y, pmd->weight, -125, 125, attr | GVAR_250, 0 ) ;
	            ...
	            putsDrSwitches( 17*FW+2, 2*FH , pmd->swtch, 0 ) ;
...
//							if ( value || pmd->curve )
//							{
								lcd_putsAttIdx( 13*FW+2, 3*FH, XPSTR("\003CveDifExp"), value, 0 ) ;
								switch ( value )
								{
									case 0 :
										put_curve( 18*FW+2, 3*FH, pmd->curve, 0 ) ;
									break ;
									case 1 :
				      	    gvarMenuItem( FW*21+1, 3*FH, pmd->curve, -100, 100, 0, 0 ) ;
									break ;
									case 2 :
            			lcd_outdezAtt( FW*21+1, 3*FH, pmd->curve + 128, 0 ) ;
									break ;
								}
//							}
							uint8_t b = 1 ;
							uint8_t z = pmd->modeControl ;
							lcd_puts_P( 13*FW+2, 4*FH, XPSTR("M") ) ;
							for ( uint8_t p = 0 ; p<MAX_MODES+1 ; p++ )
							{
								if ( ( z & b ) == 0 )
								{
    							lcd_putcAtt( (14+p)*FW+2, 4*FH, '0'+p, 0 ) ;
								}
								b <<= 1 ;
							}

							lcd_puts_P( 13*FW+2, 6*FH, XPSTR("D    :") ) ;
						  lcd_outdezAtt( FW*18+2, 6*FH, pmd->delayUp, PREC1 ) ;
						  lcd_outdezAtt( FW*21+1, 6*FH, pmd->delayDown, PREC1 ) ;

							lcd_puts_P( 13*FW+2, 7*FH, XPSTR("S    :") ) ;
						  lcd_outdezAtt( FW*18+2, 7*FH, pmd->speedDown, PREC1 ) ;
						  lcd_outdezAtt( FW*21+1, 7*FH, pmd->speedUp, PREC1 ) ;
						}					 	
						if ( s_moveMode )
						{
							if ( s_moveMixIdx == mix_index )
							{
								lcd_char_inverse( 4*FW, y, 76-4*FW, 0 ) ;
								s_currMixIdx = mix_index ;
...
void menuBindOptions(uint8_t event)
{
  ...
  switch(event)
  {
    ...
    case EVT_KEY_FIRST(KEY_MENU):
		  ...
		  {
	  		PxxFlag[s_currIdx] = PXX_BIND ;		    	//send bind code
	...
	b = checkIndexed( 2*FH, XPSTR("\0""\001""\012Chans 1-8 Chans 9-16"), b, (sub==1) ) ;
	PxxExtra[s_currIdx] =	( PxxExtra[s_currIdx] & ~1 ) | b ;
	b = (PxxExtra[s_currIdx] & 2) ? 1 : 0 ;
	b = checkIndexed( 3*FH, XPSTR("\0""\001""\014Telemetry   No Telemetry"), b, (sub==2) ) ;
...
#ifdef PCBSKY
 #ifdef REVX
const uint8_t ProtocolOptions[2][6] = { {3,0,2,3}, {5,0,1,2,3,4} };
 #else
const uint8_t ProtocolOptions[2][5] = { {3,0,2,3}, {4,0,1,2,3} };
 #endif
#endif
#ifdef PCB9XT
...
void checkProtocolOptions( uint8_t module )
{
	...
	if ( module == 0 )
	{
		g_model.Module[module].protocol = 1 ;
		return ;
	}
#endif
...
//	g_model.Module[module].protocol = index ;
	...
	g_model.Module[module].protocol = checkOutOfOrder( index, options, count ) ;
...
//#ifdef BASIC
//uint16_t BasicDebug1 ;
//uint16_t BasicDebug2 ;
//uint16_t BasicDebug3 ;
//uint16_t BasicDebug4 ;
//uint16_t BasicDebug5 ;
//uint16_t BasicDebug6 ;
//uint16_t BasicDebug7 ;
...
//	lcd_outhex4( 0, 5*FH, BasicDebug5 ) ;
//	lcd_outhex4( 30, 5*FH, BasicDebug6 ) ;
//	lcd_outhex4( 60, 5*FH, BasicDebug7 ) ;
...
//	lcd_outhex4( 90, 7*FH, (LoadingIndex<<8) | BasicDebug4 ) ;
...
#if defined(PCBSKY) || defined(PCB9XT)
//extern uint8_t YearDebug[64] ;

//	uint8_t *p = YearDebug ;
//	lcd_outhex4( 0, 1*FH, p[1] | (p[0]<<8) ) ;
//	p += 2 ;
//	lcd_outhex4( 25, 1*FH, p[1] | (p[0]<<8) ) ;
//	p += 2 ;
//	lcd_outhex4( 50, 1*FH, p[1] | (p[0]<<8) ) ;
//	p += 2 ;
//	lcd_outhex4( 75, 1*FH, p[1] | (p[0]<<8) ) ;
//	p += 2 ;
//	lcd_outhex4( 100, 1*FH, p[1] | (p[0]<<8) ) ;
//	p += 2 ;
//	lcd_outhex4( 0, 2*FH, p[1] | (p[0]<<8) ) ;
//	p += 2 ;
//	lcd_outhex4( 25, 2*FH, p[1] | (p[0]<<8) ) ;
//	p += 2 ;
//	lcd_outhex4( 50, 2*FH, p[1] | (p[0]<<8) ) ;
//	p += 2 ;
//	lcd_outhex4( 75, 2*FH, p[1] | (p[0]<<8) ) ;
//	p += 2 ;
//	lcd_outhex4( 100, 2*FH, p[1] | (p[0]<<8) ) ;
//	p += 2 ;
//	lcd_outhex4( 0, 3*FH, p[1] | (p[0]<<8) ) ;
//	p += 2 ;
//	lcd_outhex4( 25, 3*FH, p[1] | (p[0]<<8) ) ;
//	p += 2 ;
//	lcd_outhex4( 50, 3*FH, p[1] | (p[0]<<8) ) ;
//	p += 2 ;
//	lcd_outhex4( 75, 3*FH, p[1] | (p[0]<<8) ) ;
//	p += 2 ;
//	lcd_outhex4( 100, 3*FH, p[1] | (p[0]<<8) ) ;
//	p += 2 ;
//	lcd_outhex4( 0, 4*FH, p[1] | (p[0]<<8) ) ;
//	p += 2 ;
//	lcd_outhex4( 25, 4*FH, p[1] | (p[0]<<8) ) ;
//	p += 2 ;
//	lcd_outhex4( 50, 4*FH, p[1] | (p[0]<<8) ) ;
//	p += 2 ;
//	lcd_outhex4( 75, 4*FH, p[1] | (p[0]<<8) ) ;
//	p += 2 ;
//	lcd_outhex4( 100, 4*FH, p[1] | (p[0]<<8) ) ;
//	p += 2 ;
//	lcd_outhex4( 0, 5*FH, p[1] | (p[0]<<8) ) ;
//	p += 2 ;
//	lcd_outhex4( 25, 5*FH, p[1] | (p[0]<<8) ) ;
//	p += 2 ;
//	lcd_outhex4( 50, 5*FH, p[1] | (p[0]<<8) ) ;
//	p += 2 ;
//	lcd_outhex4( 75, 5*FH, p[1] | (p[0]<<8) ) ;
//	p += 2 ;
//	lcd_outhex4( 100, 5*FH, p[1] | (p[0]<<8) ) ;
//	p += 2 ;
...
extern uint16_t HbeatCounter ;
  lcd_outhex4( 0, 3*FH, HbeatCounter ) ;
#endif
//#if defined(PCBX9D) || defined(PCBX12D)
#if defined(PCBX9D)
...
//	lcd_outhex4( 0,  4*FH, FrskyHubMaxMin.hubMax[0] ) ;
//	lcd_outhex4( 30, 4*FH, FrskyHubMaxMin.hubMax[1] ) ;
//	lcd_outhex4( 60, 4*FH, FrskyHubMaxMin.hubMax[2] ) ;
//	lcd_outhex4( 90, 4*FH, FrskyHubMaxMin.hubMax[3] ) ;
//	lcd_outhex4( 0,  5*FH, FrskyHubMaxMin.hubMax[4] ) ;
//	lcd_outhex4( 30, 5*FH, FrskyHubMaxMin.hubMax[5] ) ;
//	lcd_outhex4( 60, 5*FH, FrskyHubMaxMin.hubMax[6] ) ;
//	lcd_outhex4( 90, 5*FH, FrskyHubMaxMin.hubMax[7] ) ;



//extern uint16_t UserErrorCount ;
//  lcd_outhex4( 0*FW, 7*FH, UserErrorCount ) ;

//extern uint16_t TempChannels[8] ;
...
//	{
//		uint8_t j = i + '0' ;
//		uint8_t y = i > 11 ? 5*FH : 4*FH ;
//		uint8_t x = i > 11 ? i - 12 : i ;
//		x *= FW ;
//		if ( i > 9 )
//		{
//			j += 7 ;
...
//extern uint32_t DebugCom2Diff ;
//	lcd_outhex4( 96, 6*FH, DebugCom2Diff ) ;
...
#if defined(PCBSKY) || defined(PCBX9D) || defined(PCB9XT) || defined(PCBX12D)
 #ifndef REVA
		disp_datetime( 1*FH ) ;
 #endif
...
#else // PCBX7
#if defined(PCBX12D)    
			putHwSwitchName((2+j*(15+4))*FW-2 + X12SMALL_OFFSET, y, i, 0 ) ;
#else
...
#endif // PCBX7
#ifdef PCBX12D
			lcd_putc((4+j*(15+4))*FW-2 + X12SMALL_OFFSET, y, PSTR(HW_SWITCHARROW_STR)[pos] ) ;
#else
		...
		else
		{
...
#endif
			{
...
#endif	// REV9E
				{
					...
					k += switchPosition(k) ;
#ifdef PCBX12D
					putSwitchName((2+j*(15+4))*FW-2 + X12SMALL_OFFSET, y, k-HSW_OFFSET, 0) ;
#else
...
#else // PCBX7
#ifdef PCBX12D
			putSwitchName((2+j*(15+4))*FW-2 + X12SMALL_OFFSET, y, i+1-m, getSwitch00(i+2-m) ? INVERS : 0 ) ;
#else
...
#define TL 27
  	//                        LH LV RV RH
#if defined(PCBX12D)
  	static uint8_t x[4]    = {128*1/4+2 + X12SMALL_OFFSET, 4 + X12SMALL_OFFSET, 128+24-4 + X12SMALL_OFFSET, 128*3/4-2+24 + X12SMALL_OFFSET};
#else
...
extern uint8_t ModelImageValid ;
	...
	extern void lcd_picture( uint16_t i_x, uint16_t i_y ) ;
			lcd_picture( 480-128, 128 ) ;
		}
		else
		{
			lcdDrawSolidFilledRectDMA( 480-128, 128, 128, 64, 0 ) ;
			uint8_t oldPlotType = plotType ;
			plotType = PLOT_WHITE ;
			lcd_vline( 240-64, 64, 32 ) ;
			lcd_vline( 239, 64, 32 ) ;
			lcd_hline( 240-64, 64, 64 ) ;
			lcd_hline( 240-64, 95, 64 ) ;
			plotType = oldPlotType ;
			lcd_putsAtt( 240-64+26, 68, "No", INVERS ) ;
			lcd_putsAtt( 240-64+26-9, 76, "Image", INVERS ) ;
		}
		PictureDrawn += 1 ;
	}
#endif
...
//	lcd_outhex4( 90, 64, GPIOB->ODR ) ;
//	lcd_outhex4( 0, 72, GPIOB->AFR[0] ) ;


//extern void lcdDrawCharBitmapDma( uint16_t x, uint16_t y, uint8_t chr, uint32_t mode, uint16_t background ) ;
//	lcdDrawCharBitmapDma( 10, 70, 'A', CONDENSED, LCD_WHITE ) ;
//	lcd_outhex4( 10, 78, 0x9AB3 ) ;

//	uint32_t xi ;
//	for ( xi = 0 ; xi < 26 ; xi += 1 )
//	{
//		lcdDrawCharBitmapDma( xi*6, 66, 'A'+xi, 0 ) ;
//		lcdDrawCharBitmapDma( xi*6, 66+8, 'A'+xi, 0 ) ;
//		lcdDrawCharBitmapDma( xi*6, 66+16, 'A'+xi, 0 ) ;
//		lcdDrawCharBitmapDma( xi*6, 66+24, 'A'+xi, 0 ) ;
//		lcdDrawCharBitmapDma( xi*6, 66+32, 'A'+xi, 0 ) ;
//		lcdDrawCharBitmapDma( xi*6, 66+40, 'A'+xi, 0 ) ;

//  	lcd_putc(xi*6, 66, 'A'+xi) ;
//  	lcd_putc(xi*6, 66+8, 'A'+xi) ;
//  	lcd_putc(xi*6, 66+16, 'A'+xi) ;
//  	lcd_putc(xi*6, 66+24, 'A'+xi) ;
//  	lcd_putc(xi*6, 66+32, 'A'+xi) ;
//  	lcd_putc(xi*6, 66+40, 'A'+xi) ;



//	}
	
//	lcdDrawCharBitmapDma( 10, 90, 'A', 1 ) ;

//extern uint32_t FileSizeGeneral ;
//	lcd_outhex4( 0, 8*FH, FileSizeGeneral ) ;

//	lcd_outhex4( 0, 8*FH, USART2->BRR ) ;
//	lcd_outhex4( 0, 9*FH, USART2->CR1 ) ;
//	lcd_outhex4( 0, 10*FH, GPIOD->MODER ) ;
//	lcd_outhex4( 0, 11*FH, GPIOD->AFR[0] >> 16 ) ;
//extern uint16_t RxIntCount ;
//	lcd_outhex4( 0, 12*FH, RxIntCount ) ;
//	lcd_outhex4( 0, 13*FH, USART2->SR ) ;

//	lcd_outhex4( 30, 8*FH, TIM12->CNT ) ;
//	lcd_outhex4( 30, 9*FH, TIM12->DIER ) ;
//	lcd_outhex4( 30, 10*FH, TIM12->ARR ) ;
//	lcd_outhex4( 30, 11*FH, TIM12->CCR1 ) ;
//	lcd_outhex4( 30, 12*FH, TIM12->CCR2 ) ;
//	lcd_outhex4( 30, 13*FH, TIM12->CR1 ) ;

//extern uint8_t PxxSerial[] ;
//	lcd_outhex4( 60, 8*FH,  PxxSerial[0] ) ;
//	lcd_outhex4( 60, 9*FH,  PxxSerial[1] ) ;
//	lcd_outhex4( 60, 10*FH, PxxSerial[2] ) ;
//	lcd_outhex4( 60, 11*FH, PxxSerial[3] ) ;
//	lcd_outhex4( 60, 12*FH, PxxSerial[4] ) ;
//	lcd_outhex4( 60, 13*FH, PxxSerial[5] ) ;
//	lcd_outhex4( 60, 14*FH, PxxSerial[6] ) ;
//extern uint16_t DbgC1 ;
//extern uint16_t DbgC2 ;
//	lcd_outhex4( 60, 15*FH, DbgC1 ) ;
//	lcd_outhex4( 60, 16*FH, DbgC2 ) ;
...
//	ppp = ppmStream[EXTERNAL_MODULE] ;
//	uint32_t iii ;
//	uint32_t yyy ;
//	yyy = 8 * FH ;
//	for ( iii = 0 ; iii < 8 ; iii += 1 )
//	{
//		lcd_outhex4( 0, yyy, *ppp ) ;
//		ppp += 1 ;
//		yyy += FH ;
//	}
...
//	if ( isProdVersion() )
//	{
//		lcd_outhex4( 30, 8*FH, TIM1->CNT ) ;
//		lcd_outhex4( 30, 9*FH, TIM1->DIER ) ;
//		lcd_outhex4( 30, 10*FH, TIM1->ARR ) ;
//		lcd_outhex4( 30, 11*FH, TIM1->CCR1 ) ;
//		lcd_outhex4( 30, 12*FH, TIM1->CCR2 ) ;
//		lcd_outhex4( 30, 13*FH, TIM1->CR1 ) ;
//	}
//	else
//	{
//		lcd_outhex4( 60, 8*FH,  TIM2->CNT ) ;
//		lcd_outhex4( 60, 9*FH,  TIM2->DIER ) ;
//		lcd_outhex4( 60, 10*FH, TIM2->ARR ) ;
//		lcd_outhex4( 60, 11*FH, TIM2->CCR1 ) ;
//		lcd_outhex4( 60, 12*FH, TIM2->CCR2 ) ;

//		lcd_outhex4( 60, 13*FH, TIM2->CR1 ) ;
//		lcd_outhex4( 60, 14*FH, TIM2->SMCR ) ;
		
//		lcd_outhex4( 30, 8*FH, TIM2->CNT >> 16 ) ;
...
//	lcd_outhex4( 90, 9*FH, pulseStreamCount[1] ) ;
//	lcd_outhex4( 90, 10*FH, TIM1->CNT ) ;
//	lcd_outhex4( 90, 11*FH, TIM1->DIER ) ;
//	lcd_outhex4( 90, 12*FH, TIM1->ARR ) ;
//	lcd_outhex4( 90, 13*FH, TIM1->CCR1 ) ;
//	lcd_outhex4( 90, 14*FH, TIM1->CCR2 ) ;
//	lcd_outhex4( 90, 15*FH, TIM1->CCR3 ) ;
//	lcd_outhex4( 90, 16*FH, TIM1->CR2 ) ;
	
	
//	lcd_outhex4( 150, 10*FH, EXTMODULE_DMA_STREAM->CR ) ;
//	lcd_outhex4( 150, 11*FH, EXTMODULE_DMA_STREAM->NDTR ) ;
//	lcd_outhex4( 150, 12*FH, EXTMODULE_DMA_STREAM->PAR ) ;
//	lcd_outhex4( 150, 13*FH, EXTMODULE_DMA_STREAM->M0AR ) ;
//	lcd_outhex4( 150, 14*FH, EXTMODULE_DMA_STREAM->M0AR >> 16 ) ;

//extern uint16_t ProtoDebug[] ;
//	lcd_outhex4( 120, 8*FH,  ProtoDebug[0] ) ;
//	lcd_outhex4( 120, 9*FH,  ProtoDebug[1] ) ;
//	lcd_outhex4( 120, 10*FH, ProtoDebug[2] ) ;
//	lcd_outhex4( 120, 11*FH, ProtoDebug[3] ) ;
//	lcd_outhex4( 120, 12*FH, ProtoDebug[4] ) ;
//	lcd_outhex4( 120, 13*FH, ProtoDebug[5] ) ;
//	lcd_outhex4( 120, 14*FH, ProtoDebug[6] ) ;
//	lcd_outhex4( 120, 15*FH, ProtoDebug[7] ) ;
//	lcd_outhex4( 120, 16*FH, ProtoDebug[8] ) ;

//extern uint16_t pxxStream[][400];

//	uint16_t temp[12] ;
//	temp[0] = pxxStream[EXTERNAL_MODULE][0] ;
//	temp[1] = pxxStream[EXTERNAL_MODULE][1] ;
//	temp[2] = pxxStream[EXTERNAL_MODULE][2] ;
//	temp[3] = pxxStream[EXTERNAL_MODULE][3] ;
//	temp[4] = pxxStream[EXTERNAL_MODULE][4] ;
//	temp[5] = pxxStream[EXTERNAL_MODULE][5] ;

//	lcd_outhex4( 180, 8*FH,  temp[0] ) ;
//	lcd_outhex4( 180, 9*FH,  temp[1] ) ;
//	lcd_outhex4( 180, 10*FH, temp[2] ) ;
...
static uint8_t indexProcess( uint8_t event, MState2 *pmstate, uint8_t extra )
{
	...
	if ( SubmenuIndex )
	{
  	if ( (event == EVT_KEY_BREAK(KEY_EXIT) ) || ( (event == EVT_KEY_LONG(BTN_RE) ) && ( MaskRotaryLong == 0 ) ) )
		{
      if(s_editMode)
			{
        s_editMode = false;
...
//#define BT_TYPE_HC05		1
	...
	EditType = EE_MODEL ;
			...
			g_model.BTfunction = checkIndexed( y, XPSTR(FWx13"\003""\007    OFF  TrnRxTrnTxRxLcdDump"), g_model.BTfunction, (sub==subN) ) ;
			y += FH ;
			subN += 1 ;
	EditType = EE_GENERAL ;
...
//extern uint8_t BtDebug[] ;

void menuProcBt(uint8_t event)
{
  MENU(XPSTR("BT"), menuTabStat, e_bluetooth, 7, {0/*, 0*/});

//	lcd_outhex4( 30, 0, BtDebug[0] | (BtDebug[1]<<8) ) ;
//	lcd_outhex4( 60, 0, BtDebug[2] | (BtDebug[3]<<8) ) ;
...
void displayStatusLine()
{
	...
	putsTime( 224, (STATUS_VERTICAL/2)+2, Time.hour*60+Time.minute, 0, 0, LCD_STATUS_GREY ) ;

	lcd_img( 180, (STATUS_VERTICAL/2)+2, speaker, 0, 0, LCD_BLACK, LCD_STATUS_GREY ) ;
extern uint8_t CurrentVolume ;
	plotType = PLOT_BLACK ;
	lcd_hbar( 185, (STATUS_VERTICAL/2)+3, 23, 6, (CurrentVolume*100+16)/23 ) ;
	plotType = oldPlotType ;
...
extern uint32_t IdlePercent ;
  lcd_outdezAtt( 4*FW, (STATUS_VERTICAL/2)+2, IdlePercent ,PREC2, LCD_STATUS_GREY ) ;
}
#endif
