...
const uint8_t TelemSize = sizeof(TelemIndex) ;
...
void displayGPSdata( uint16_t x, uint16_t y, uint16_t whole, uint16_t frac, uint16_t attr, uint8_t direction )
{
	...
	{
		int32_t value ;
	  lcd_outdezNAtt(x+3*chwidth+2, y, qr.quot, attr, -3);
  	lcd_putcAtt(x+3*chwidth+2, y, '.', attr ) ;
		value = qr.rem ;
		value *= 10000 ;
		value += frac ;
		value *= 10 ;
		value /= 6 ;
    lcd_outdezNAtt(x+9*chwidth, y, value, attr, -6);
		lcd_putcAtt( x+10*chwidth, y, direction, attr ) ;
	}
	else
	{
    lcd_outdezNAtt(x+3*chwidth, y, qr.quot, attr, -3);
    lcd_outdezNAtt(x+6*chwidth-3, y, qr.rem, attr, -2);
    lcd_putcAtt(x+6*chwidth-3, y, '.', attr ) ;
    lcd_outdezNAtt(x+10*chwidth-3, y, frac, attr, -4);
		lcd_putcAtt( x+11*chwidth-3, y, direction, attr ) ;
	}
}
...
void displayMultiProtocol( uint32_t index, uint32_t y, uint8_t attr )
{
	...
	if ( PrivateData[1] )
	{
		...
		{
#ifdef TOUCH
	  	lcd_putsAttColour( x-FW*5, y, XPSTR("N/A"), BLINK, active ? ~LcdForeground : LcdForeground ) ;
#else
			lcd_putsAtt( x+FW*8, y, "N/A", BLINK ) ;
#endif
		}
	}
#ifdef PCBT16
	...
	{
		lcd_putsAtt( x, y, (const char *)MultiSetting.protocol, attr ) ;
		return ;
	}
#ifdef PCBT16
	}
...
#endif
			value &= 0x07FF ;
			lcd_putsAtt( x, y, &p[value], attr ) ;
			return ;
		}
	}
	lcd_outdezAtt( x+7*FW, y, index, attr ) ;
}
...
uint32_t displayMultiSubProtocol( uint32_t index, uint32_t subIndex, uint32_t y, uint8_t attr )
{
	...
	{
		lcd_putsAtt( x, y, (const char *)MultiSetting.subProtocol, attr ) ;
		return subIndex ;
	}
#ifdef PCBT16
	}
	...
	if ( index <= NUM_MULTI_PROTOCOLS )
	{
		...
		if ( value )
		{
			...
			if ( subIndex < limit )
			{
				...
				while ( value-- ) ;
				lcd_putsAtt( x, y, (char *)list, attr ) ;
			}
			else
			{
				if ( subIndex == 0 )
				{
					lcd_putsAtt( x, y, "None", attr ) ;
	//				return 0 ;
				}
				else
				{
#ifdef TOUCH
					lcd_outdezAtt( x+10*FW, y, subIndex, attr ) ;
#else
					lcd_outdezAtt( x+11*FW, y, subIndex, attr ) ;
#endif
				}
			}
			return subIndex ;
	...
	lcd_outdezAtt( x+1*FW, y, subIndex, attr ) ;
	return subIndex ;
}
...
//			lcd_puts_P( 718, 26, "EXIT" ) ;
 ...
 if ( maxrow != 0xFF )
 {
  ...
  if(!s_editMode)
	{
//		if ( TouchUpdated && !TlExitIcon && (NoCheckSelect==0) )
...
#ifdef TOUCH
		lcd_outdezAtt(12*FW, 0, s_curveChan+1, INVERS);
#else
  ...
  for(uint8_t i=0; i<4; i++)
	{
		if(!IS_THROTTLE(i))
		{
			...
			{
				int16_t v = rawSticks[i] / 2 ;
				int16_t original_trim = getTrimValueAdd(thisPhase, i);
				if ( ( v > 10 ) || ( v < -10 ) )
				{
					v = max(min( original_trim + v,125),-125) ;
			...
			{
				int16_t original_trim = getTrimValueAdd(thisPhase, i);
      	for (uint8_t phase=0; phase<MAX_MODES+1; phase +=  1)
				{
  ...
  for( CPU_UINT j=0 ; j < SCREEN_LINES-1 ; j += 1 )
    ...
    uint8_t attr = (sub==k) ? blink : 0 ;
		...
		else if ( k == 1 )
		{
			...
			lcd_putsAttIdx( 15*FW+LOG_OFF_0, y, XPSTR("\0041.0s2.0s0.5s0.2s"), g_model.logRate, attr ) ;
 			if(attr)
			{
		...
		{
			lcd_putsAtt( 0+LOG_OFF_0, y, XPSTR("Select None"), attr ) ;
			if ( attr )
			{
#ifdef TOUCH
		...
		{
			lcd_putsAtt( 0+LOG_OFF_0, y, XPSTR("Select All"), attr ) ;
			if ( attr )
			{
#ifdef TOUCH
				uint32_t activated = handleSelectIcon() ;
		...
		{
			lcd_putsAtt( 0+LOG_OFF_0, y, XPSTR("Select Active"), attr ) ;
			if ( attr )
			{
#ifdef TOUCH
				uint32_t activated = handleSelectIcon() ;
...
////					}
					if ( attr )
					{
						putsTelemetryChannel( 12*FW+LOG_OFF_0, 0, idx, get_telemetry_value(idx), 0, TELEM_UNIT ) ;
					}
				}
				index = idx ;
...
//				}
  ...
  index = s_pgOfs ;
	...
	{
		uint32_t newVpos ;
		newVpos = scrollBar( 380, 16, 42, 240-16, rows+1-(TLINES-2)+1, s_pgOfs ) ;
		if ( newVpos != s_pgOfs )
		{
			s_pgOfs = newVpos ;
			if ( sub < s_pgOfs )
...
#endif
	{
		...
		else if ( type == TEXT_TYPE_SW_SOURCE )
		{
			...
			putsChnRaw( x, y, unmapPots(tidx), att | TSSI_TEXT ) ;
		}
		else if ( type == TEXT_TYPE_MIX_SOURCE )
		{
			uint8_t idx ;
...
//				}
//			}
			putsChnOpRaw( x, y, idx, swidx, TextCtrl->TextOption, att ) ;
		}
		else if ( type == TEXT_TYPE_SW_FUNCTION )
		{
		...
		else if ( type == TEXT_TYPE_CUSTOM )
		{
			...
			{
				(*TextCtrl->TextFunction)( x, y, index, att ) ;
			}
...
//		b = TelOptions[b] ;
    ...
    lcd_putc(FW, y, '1'+i);
		...
		else
		{
			...
			lcd_outdezAtt( 21*FW, y, TelemetryData[FR_A3+index], PREC2 ) ;
...
//	lcd_outdezAtt( 21*FW, y, convertRxv( TelemetryData[FR_RXV] ), PREC1 ) ;
	lcd_outdezAtt( 21*FW, y, TelemetryData[FR_RXV], PREC1 ) ;
    ...
    uint8_t attr ;
  	lcd_puts_Pleft( y, XPSTR( "Cell\023v" ) ) ;
		...
		attr = TelemetryDataValid[index] ? 0 : BLINK ;
 		lcd_outdezAtt(  19*FW-2, y, TelemetryData[index], attr | PREC2 ) ;
    ...
    uint8_t attr = ((sub==k) ? InverseBlink : 0);
		...
		if ( k == 0 )
		{
		  lcd_puts_P( FAIL_OFF_0, y, XPSTR( "Mode" ) ) ;
			EditColumns = 1 ;
			g_model.Module[module].failsafeMode = checkIndexed( y, XPSTR(FWx13"\004""\007Not Set     Rx Custom   HoldNoPulse"), g_model.Module[module].failsafeMode, attr ) ;
			EditColumns = 0 ;
		}
		else if ( k == 1 )
		{
  ...
  uint8_t k ;
    ...
    k=i+t_pgOfs;
  	...
  	lcd_putc( 0+ADJ_OFF_0, y, 'A' ) ;
		...
		{
			int16_t value ;
 			lcd_putc( 16*FW+ADJ_OFF_0, 0, '=' ) ;
//  		lcd_puts_Pleft( 0, XPSTR("\020=") ) ;
			value = 7 ;
//#if MULTI_GVARS
//			if ( g_model.flightModeGvars )
//			{
...
//#endif
			if ( idx >= value )
			{
				lcd_putsAttIdx( 13*FW+ADJ_OFF_0, 0, PSTR(STR_GV_SOURCE), idx-(value-1), 0 ) ;
...
//#endif
				{
//#if MULTI_GVARS
//					lcd_putsAttIdx( 3*FW+2+ADJ_OFF_0, y, PSTR(STR_GV_SOURCE), idx-(g_model.flightModeGvars ? 11 : 6), attr ) ;
//#else
...
//#endif
				...
				{
					dispGvar( 3*FW+2+ADJ_OFF_0, y, idx+1, attr ) ;
				}
				if ( active )
				{
//#if MULTI_GVARS
...
//#endif
			...
			else
			{
				if ( ( pgvaradj->function < 4 ) || ( pgvaradj->function > 6 ) )
				{
					...
					{
						lcd_putsAttIdx( 18*FW+ADJ_OFF_0, y, PSTR(STR_GV_SOURCE), pgvaradj->switch_value, attr ) ;
#if defined(PCBSKY) || defined(PCB9XT)
...
//			case 2 :
//  	  	lcd_puts_Pleft( y, XPSTR("Precision") ) ;
...
//			case 3 :
//  	  	lcd_puts_Pleft( y, XPSTR("Minimum") ) ;
//				lcd_outdezAtt( FW*20, y, GVAR_MIN+pgvar->min, attr ) ;
//				if ( attr )
//				{
...
//			case 4 :
//  	  	lcd_puts_Pleft( y, XPSTR("Maximum") ) ;
//				lcd_outdezAtt( FW*20, y, GVAR_MAX-pgvar->max, attr ) ;
//	    	if(attr)
//				{
...
//	#endif
    ...
    y=(i+1)*FH;
		...
		{
      uint8_t attr = ((subN==k && subSub==j) ? InverseBlink : 0);
			GvarData *pgvar ;
//#if MULTI_GVARS
//			if ( g_model.flightModeGvars )
//			{
...
//#endif
      ...
      if ( j == 0 )
			{
       	putsDrSwitches( 6*FW+GLO_OFF_0, y, g_model.gvswitch[k] ,attr );
...
// #endif
				...
				if ( t >= EXTRA_POTS_START )
				{
					lcd_putsAttIdx( 12*FW-4+GLO_OFF_0, y, PSTR(STR_CHANS_EXTRA), t - EXTRA_POTS_START, attr ) ;
				}
				else
				{
#ifdef PCBT12
					...
					{
						lcd_putsAttIdx( 12*FW-4+GLO_OFF_0, y, "\004AUX4AUX5", t - 10, attr ) ;
					}
					else
#endif 
					lcd_putsAttIdx( 12*FW-4+GLO_OFF_0, y, PSTR(STR_GV_SOURCE), t, attr ) ;
				}
					// STR_GV_SOURCE
  			if(active)
				{
...
//				else
//				{
//					lcd_outdezAtt( 19*FW+GLO_OFF_0, y, readMgvar( 0, k ), 0 ) ;
//				}
//#endif
			}
		}
...
//	    putsChn(0+SAFE_OFF_0,y,k,attr);
			...
			if ( k <= numSafety )
			{
		    lcd_putsnAtt(0+SAFE_OFF_0,y,PSTR(STR_CH),2,attr);
...
//	uint8_t x = TITLEP( PSTR(STR_EDIT_MIX));
	...
	lcd_puts_Pleft( FH, XPSTR("Function\037\037\037AND\037Delay\037Ex.Func\037Copy") ) ;

  if( (cstate == CS_VOFS) || (cstate == CS_2VAL) )
  {
		lcd_puts_Pleft( 2*FH, XPSTR("v1\037val") ) ;
  ...
  {
		lcd_puts_Pleft( 2*FH, XPSTR("v1\037v2") ) ;		
...
//#endif
							if ( pvad->file.vfile >= t )
							{
								...
								lcd_putcAtt( 20*FW, y, pvad->file.vfile - t + '0', attr ) ;
							}
							else
							{
...
uint8_t editSlowDelay( uint8_t y, uint8_t attr, uint8_t value)
{
  ...
  lcd_outdezAtt(TRIGHT-TRMARGIN,y+TVOFF,lval,PREC1) ;
	restoreEditColours() ;
#else
  lcd_outdezAtt(FW*20-3,y,lval,attr|PREC1);
#endif
	return value ;
}
          ...
          lcd_puts_P(  2*FW,y,PSTR(STR_2WEIGHT));
					...
					if ( g_model.vars )
					{
						...
						else
						{
							...
							value = editVarCapableValue( FW*18, y, value, -350, 350, NUM_VARS, attr, event ) ;
							if ( value > 900 )
							{
								value -= 1000 ;
								md2->varForWeight = 1 ;
...
//            lcd_puts_Pleft( y,XPSTR("\001MODES"));
        ...
        case 10:
            ...
            if(b)
                ...
                lcd_putsAtt(  FW*17,y,PSTR(STR_OFF),attr);
            if(attr) { CHECK_INCDEC_H_MODELVAR_0( b, 3); md2->mixWarn = b ; }
            break;
        case 11:
            lcd_puts_P(  2*FW,y,PSTR(STR_2MULTIPLEX));
            lcd_putsAttIdx(12*FW, y,PSTR(STR_ADD_MULT_REP),md2->mltpx,attr);
...
#ifdef TOUCH
	drawButton( 3*FW-5, 5*FH, 7*FW+10, 2*FH ) ;
	drawButton( 11*FW-5, 5*FH, 6*FW+10, 2*FH ) ;
#endif	
...
#ifdef TOUCH
	if ( checkTouchArea( 4*FW, 5*FH, 5*FW, 2*FH ) > 0 )
	{
		event = EVT_KEY_BREAK(BTN_RE) ;
	}

	if ( checkTouchArea( 12*FW, 5*FH, 5*FW, 2*FH ) > 0 )
	{
...
//			uint32_t len = strlen( list ) ;
			lcd_puts_P( x+FW, y, (const char *)(list) ) ;
...
static uint32_t popupDisplay( const char *list, uint16_t mask, uint8_t width )
{
	...
	lcdDrawSolidFilledRectDMA( (3*FW + X12OFFSET+1)*2, POPUP_VOFFSET*2, width*FW*2-2, (FH + entries*(FH))*2, LcdBackground ) ;
    	...
    	do
			{
        	  ...
        	  if(firstMix != mix_index) //show prefix only if not first mix
        	 		...
        	 		lcd_putsAttIdx( 4*FW-8, y, XPSTR("\001+*R"),pmd->mltpx,0 ) ;
					    ...
					    lcd_outdez( 11*FW+4*FW, 0, value/2 + 1500 ) ;
#else
...
extern uint8_t swOn[] ;
						...
						{
#if defined(PCBX12D) || defined(PCBX10)
							displayVarName( 4*FW, y, lweight, attr ) ;
#else
...
#else
							lcd_puts_P( 13*FW+2+x, FH, XPSTR("Of") ) ;
#endif
							if ( pmd->varForOffset )
							{
								int16_t loffset = calcExtendedValue( pmd->sOffset, pmd->extOffset ) ;
#if defined(PCBX12D) || defined(PCBX10)
								displayVarName( FW*18+1+x, FH, loffset, 0 ) ;
#else
								displayVarName( FW*33/2-1+x, FH, loffset, LUA_SMLSIZE ) ;
#endif
							}
							else
							{
...
#else
    							lcd_putcAtt( (13+p)*FW+2+x, 4*FH, '0'+p, 0 ) ;
#endif
									modeCount += 1 ;
									modeIndex = p ;
								}
								...
								if ( modeIndex )
								{
  								...
  								PhaseData *phase ;
									...
									if ( phase->name[0] )
									{
										...
										lcd_putsnAtt( 14*FW+2+x, 5*FH, phase->name, 6, /*BSS*/ 0 ) ;
									}
								}
							}
...
#if defined(PCBX12D) || defined(PCBX10)
#define EXPO_LEFT_X		(7*FW)
#define EXPO_RIGHT_X	(15*FW)
#else
...
#define EXPO_RIGHT_X	(9*FW)
	...
	editExpoVals( event, (stkVal != DR_RIGHT) && (sub==subN), EXPO_LEFT_X, 3*FH, expoDrOn, DR_EXPO, DR_LEFT ) ;
//#if TOUCH
//	if ( stkVal == DR_BOTH )
//	{
//		if ( TouchAdjustValue )
...
//	}
//#endif
	editExpoVals( event, (stkVal != DR_LEFT) && (sub==subN), EXPO_RIGHT_X, 3*FH, expoDrOn, DR_EXPO, DR_RIGHT ) ;
	subN += 1 ;
	lcd_puts_Pleft(4*FH,PSTR(STR_2WEIGHT));
	editExpoVals( event, (stkVal != DR_RIGHT) && (sub==subN), EXPO_LEFT_X, 5*FH, expoDrOn, DR_WEIGHT, DR_LEFT ) ;
//#if TOUCH
//	if ( stkVal == DR_BOTH )
//	{
//		if ( TouchAdjustValue )
...
//	}
//#endif
	editExpoVals( event, (stkVal != DR_LEFT) && (sub==subN), EXPO_RIGHT_X, 5*FH, expoDrOn, DR_WEIGHT, DR_RIGHT ) ;
	subN += 1 ;
...
#if defined(PCBX12D) || defined(PCBX10)
	lcd_outdezAtt( XD+FW, 1*FH-2, y512*25/((signed) RESXu/4), 0 );
#else
...
{
	uint32_t retValue = 0 ;
	lcdDrawIcon( LCD_W-TICON_SIZE-2, 111, IconHselect, 0 ) ;
	if ( TouchUpdated )
	{
		if ( TouchControl.event == TEVT_UP )
		{
			if ( TouchControl.x > LCD_W-TICON_SIZE )
			{
				if ( ( TouchControl.y >= 111 ) && ( TouchControl.y <= 160 ) )
				{
					retValue = 1 ;
					TouchUpdated = 0 ;
				}
...
extern const uint8_t IconHexit[] ;
	...
	if ( binding == 0 )
	{
		lcd_outdezAtt( 12 * FW, 3*FH, TelemetryData[FR_RXRSI_COPY], DBLSIZE);
//		{
//			lcd_outdezAtt( 20 * FW, 4*FH, TelemetryData[FR_TEMP2], DBLSIZE) ;
//		}
		if ( TelemetryDataValid[FR_VFR] )
		{
			lcd_puts_Pleft( 6*FH, "VFR:" ) ;
			lcd_outdezAtt( 10*FW, 6*FH, TelemetryData[FR_VFR], DBLSIZE);
		}
	}
...
void editAccessRxOptions( uint8_t module, uint8_t event )
{
	...
	lcd_putsn_P( 16*FW, 0, (char *)g_model.Access[module].receiverName[ModuleControl[module].bindReceiverIndex], PXX2_LEN_RX_NAME ) ;
  ...
  uint8_t t_pgOfs ;
	...
	for(;;)
	{
		if(t_pgOfs<=subN)
		{
			...
			lcd_outhex4( 16*FW, y, ModuleControl[module].rxCapabilities ) ;
		...
		if(t_pgOfs<=subN)
		{
			...
			after = checkIndexed( y, XPSTR("\0""\001""\011100mW Tele 25mW Tele"), before, (sub==subN) ) ;
			if ( after != before )
			{
				needWrite = 1 ;
		...
		for ( uint32_t i = 0 ; i < ModuleControl[module].rxNumPins ; i += 1 )
		{
			if(t_pgOfs<=subN)
			{
				...
				{
					lcd_putc( 3*FW, y, '1' ) ;
					lcd_putc( 4*FW, y, i-10+'1' ) ;
				}
				uint8_t attr = (sub == subN) ? InverseBlink : 0 ;
				if ( value < 33 )
				{
					lcd_puts_P( 6*FW, y, XPSTR("CH") ) ;
					lcd_outdezAtt( 10*FW-2, y, value, attr ) ;
				}
				else
				{
					if ( value == 33 )
					{
						lcd_putsAtt( 6*FW, y, XPSTR("SBUS in"), attr ) ;
						before = 27 ;
					}
					else if ( value == 65 )
					{
						lcd_putsAtt( 6*FW, y, XPSTR("SPort"), attr ) ;
						before = 24 ;
					}
					else if ( value == 129 )
					{
						lcd_putsAtt( 6*FW, y, XPSTR("SBUS out"), attr ) ;
						before = 25 ;
					}
					else if ( value == 193 )
					{
						lcd_putsAtt( 6*FW, y, XPSTR("FBUS"), attr ) ;
						before = 26 ;
					}
					else
					{
...
void editAccessRxHardware( uint8_t module, uint8_t event )
{
	...
	if ( ModuleSettings[module].mode == MODULE_MODE_NORMAL )
	{
		...
		lcd_puts_Pleft( 5*FH, XPSTR("Capabilities") ) ;
		
		lcd_outhex4( 13*FW-1, FH, ModuleControl[module].rxHwVersion + 0x0100 ) ;
		lcd_outhex4( 13*FW-1, 2*FH, ModuleControl[module].rxSwVersion + 0x0100 ) ;
//		lcd_putc( 12*FW, 3*FH, ModuleControl[module].rxModuleId + '0' ) ;
		...
		{
			lcd_puts_P( 13*FW, 3*FH, PXX2receiversModels[index] ) ;
		}
		lcd_outhex4( 13*FW-1, 4*FH, ModuleControl[module].rxVariant ) ;
		lcd_outhex4( 13*FW-1, 5*FH, ModuleControl[module].rxCapabilities ) ;
	}
}
...
//						lcd_outdezAtt(  17*FW, y, TelemetryData[FR_RXRSI_COPY], 0 ) ;
...
void displayFlightMode( uint16_t x, uint16_t y, uint32_t mode, uint16_t attr )
{
	...
	lcd_putsAtt( x, y, XPSTR("FM"), attr ) ;
  lcd_putcAtt( x+2*FW, y, '0'+mode, attr ) ;
}
...
//#endif
  ...
  uint8_t editMode = s_editMode;
	...
	for (uint8_t i = 0 ; i < displayRows ; i += 1 )
	{
    uint16_t y = displayRows == 6 ? (i+2) * FH : (i+1) * FH ;
...
//				else
//				{
//          lcd_putsAtt( 10*FW, y, XPSTR("OWN"), g_posHorz==0 ? attr : 0) ;
//				}
//				uint32_t fm = getGVarFlightMode( s_currIdx, k ) ;

//        // Needs to be targeted gvar
//        int16_t w = readMgvar( fm, k ) ;
//				lcd_outdezAtt( 19*FW+GLO_OFF_0, y, w, g_posHorz==1 ? attr : 0) ;
  ...
  for ( i=0 ; i<MAX_MODES+1+1 ; i += 1 )
    ...
    attr = (i == sub) ? INVERS : 0 ;
  	if ( i == 0 )
		{
#if (SCREEN_LINES < 9)
		  lcd_putcAtt( PH_OFF_0 + 7*FW, 0, 'F', attr ) ;
		  lcd_putcAtt( 8*FW-1+PH_OFF_0, 0, '0', attr ) ;
			lcd_putsAtt( 16*FW+2+PH_OFF_0, 0, XPSTR("RETA"), attr ) ;
#else
		  lcd_putcAtt( PH_OFF_0, PH_Y, 'F', attr ) ;
		  lcd_putcAtt( 1*FW-1+PH_OFF_0, PH_Y, '0', attr ) ;
			lcd_putsAtt( 16*FW+2+PH_OFF_0, PH_Y, XPSTR("RETA"), attr ) ;
#endif
			s_editMode = 0 ;
			continue ;
		}
    ...
    uint8_t y = 0 + FH + k*FH ;
#else
    uint8_t y = PH_Y + FH + k*FH ;
#endif
		PhaseData *p ;
		p = getPhaseAddress(k) ;
    lcd_putc( PH_OFF_0, y, 'F' ) ;
    lcd_putc( 1*FW-1+PH_OFF_0, y, '1'+k ) ;
		lcd_putsnAtt( 2*FW+PH_OFF_0, y, p->name, 6, /*BSS*/ attr ) ;
...
//       	lcd_putcSmall( 15*FW+2+PH_OFF_0 + t*(FW+2)+2, y, '+', 0 ) ;
				pushPlotType( PLOT_BLACK ) ;
  			lcd_vline( 15*FW+2+PH_OFF_0 + t*(FW+2)+4-2, y+1, 3 ) ;
  			lcd_hline(15*FW+2+PH_OFF_0 + t*(FW+2)+3-2,y+2,3) ;
				popPlotType() ;
			}
...
//	i %= 10000 ;
	...
	if ( i > 99999 )
	{
		...
		lcd_putc( 15*FW-6*FWNUM, 0, '0'+j ) ;
	}
	lcd_outdezNAtt( 15*FW, 0, i, j ? LEADING0 : 0, 5 ) ;
...
//extern int _ebss ;
//	lcd_outhex4( 80, FH, (uint32_t)&_ebss ) ;
//#endif
...
//	lcd_outhex2( 75, FH, *p++ ) ;
//	lcd_outhex2( 90, FH, *p++ ) ;
//	lcd_outhex2( 105,FH, *p++ ) ;

//	lcd_outhex2( 0,  2*FH, *p++ ) ;
//	lcd_outhex2( 15, 2*FH, *p++ ) ;
//	lcd_outhex2( 30, 2*FH, *p++ ) ;
//	lcd_outhex2( 45, 2*FH, *p++ ) ;
//	lcd_outhex2( 60, 2*FH, *p++ ) ;
//	lcd_outhex2( 75, 2*FH, *p++ ) ;
//	lcd_outhex2( 90, 2*FH, *p++ ) ;
//	lcd_outhex2( 105,2*FH, *p++ ) ;

//	lcd_outhex2( 0,  3*FH, *p++ ) ;
//	lcd_outhex2( 15, 3*FH, *p++ ) ;
//	lcd_outhex2( 30, 3*FH, *p++ ) ;
//	lcd_outhex2( 45, 3*FH, *p++ ) ;
//	lcd_outhex2( 60, 3*FH, *p++ ) ;
//	lcd_outhex2( 75, 3*FH, *p++ ) ;
//	lcd_outhex2( 90, 3*FH, *p++ ) ;
//	lcd_outhex2( 105,3*FH, *p++ ) ;

//	lcd_outhex2( 0,  4*FH, *p++ ) ;
//	lcd_outhex2( 15, 4*FH, *p++ ) ;
//	lcd_outhex2( 30, 4*FH, *p++ ) ;
//	lcd_outhex2( 45, 4*FH, *p++ ) ;
//	lcd_outhex2( 60, 4*FH, *p++ ) ;
//	lcd_outhex2( 75, 4*FH, *p++ ) ;
//	lcd_outhex2( 90, 4*FH, *p++ ) ;
//	lcd_outhex2( 105,4*FH, *p++ ) ;

//	lcd_outhex2( 0,  5*FH, *p++ ) ;
//	lcd_outhex2( 15, 5*FH, *p++ ) ;
//	lcd_outhex2( 30, 5*FH, *p++ ) ;
//	lcd_outhex2( 45, 5*FH, *p++ ) ;
//	lcd_outhex2( 60, 5*FH, *p++ ) ;
//	lcd_outhex2( 75, 5*FH, *p++ ) ;
//	lcd_outhex2( 90, 5*FH, *p++ ) ;
//	lcd_outhex2( 105,5*FH, *p++ ) ;
//#endif
...
////extern int16_t anas[NUM_SKYXCHNRAW+1+MAX_GVARS+1] ;
//extern uint8_t InputMap[] ;

//	uint16_t y ;
//	uint32_t i ;
//	uint32_t j ;
//	uint8_t *p = &InputMap[0] ;
//	y = FH ;
//	for ( i = 0 ; i < 5 ; i += 1 )
//	{
//		for ( j = 0 ; j < 9 ; j += 1 )
//		{
//			lcd_outdezAtt( (j+1)*4*FW, y, *p++, 0 ) ;
//		}
//		y += FH ;
//	}
//	y = 8*FH ;
...
//	uint32_t i ;
//	uint32_t j = mstate2.m_posVert ;
//	for ( i = 0 ; i < 5 ; i += 1 )
//	{
//		lcd_outhex2( 0, i*FH+FH, j ) ;

//		lcd_outhex4( 12, i*FH+FH, AllAddr[j] >> 16 ) ;
//		lcd_outhex4( 35, i*FH+FH, AllAddr[j] ) ;
//		lcd_outhex4( 60, i*FH+FH, AllSize[j] ) ;
//		lcd_outhex4( 84, i*FH+FH, AllRet[j] >> 16 ) ;
//		lcd_outhex4( 107, i*FH+FH, AllRet[j] ) ;
//		j += 1 ;
		
//	}


//extern uint32_t SaveHeap[] ;
//lcd_outhex4( 0, 2*FH, SaveHeap[0] >> 16 ) ;
//lcd_outhex4( 30, 2*FH, SaveHeap[0] ) ;

//lcd_outhex4( 0, 3*FH, SaveHeap[1] >> 16 ) ;
//lcd_outhex4( 30, 3*FH, SaveHeap[1] ) ;
//lcd_outhex4( 0, 4*FH, SaveHeap[2] >> 16 ) ;
//lcd_outhex4( 30, 4*FH, SaveHeap[2] ) ;
//lcd_outhex4( 0, 5*FH, SaveHeap[3] >> 16 ) ;
//lcd_outhex4( 30, 5*FH, SaveHeap[3] ) ;

//extern unsigned char *heap ;
//lcd_outhex4( 70, 5*FH, (uint32_t)heap >> 16 ) ;
//lcd_outhex4( 100, 5*FH, (uint32_t)heap  ) ;

//lcd_outhex4( 70, 4*FH, SaveNsize >> 16 ) ;
//lcd_outhex4( 100, 4*FH, SaveNsize  ) ;
...
//extern int16_t AltOffset ;
//	lcd_outdezAtt( 7*FW, 2*FH, AltOffset, 0 ) ;

//extern uint8_t DebugTelData[] ;
//extern uint32_t DebugTelCount ;
//extern uint16_t DebugTelState ;

//	lcd_outhex4( 0, 2*FH, DebugTelCount ) ;
//	lcd_outhex4( 30, 2*FH, DebugTelState ) ;
  
//	lcd_outhex2( 0, 3*FH, DebugTelData[0] ) ;
//  lcd_outhex2( 13, 3*FH, DebugTelData[1] ) ;
//  lcd_outhex2( 26, 3*FH, DebugTelData[2] ) ;
//  lcd_outhex2( 39, 3*FH, DebugTelData[3] ) ;
//  lcd_outhex2( 52, 3*FH, DebugTelData[4] ) ;
//  lcd_outhex2( 65, 3*FH, DebugTelData[5] ) ;
//  lcd_outhex2( 78, 3*FH, DebugTelData[6] ) ;
//  lcd_outhex2( 91, 3*FH, DebugTelData[7] ) ;
//  lcd_outhex2( 104, 3*FH, DebugTelData[8] ) ;


//	lcd_outhex2( 0,  4*FH, DebugTelData[9] ) ;
//  lcd_outhex2( 13, 4*FH, DebugTelData[10] ) ;
//  lcd_outhex2( 26, 4*FH, DebugTelData[11] ) ;
//  lcd_outhex2( 39, 4*FH, DebugTelData[12] ) ;
//  lcd_outhex2( 52, 4*FH, DebugTelData[13] ) ;
//  lcd_outhex2( 65, 4*FH, DebugTelData[14] ) ;
//  lcd_outhex2( 78, 4*FH, DebugTelData[15] ) ;
//  lcd_outhex2( 91, 4*FH, DebugTelData[16] ) ;
//  lcd_outhex2( 104,4*FH, DebugTelData[17] ) ;
...
#if defined(PCBX9LITE)
//	lcd_outhex4(0,  FH, g_model.mGvars[0][0] ) ;
//	lcd_outhex4(30, FH, g_model.mGvars[0][1] ) ;
//	lcd_outhex4(60, FH, g_model.mGvars[0][2] ) ;
//	lcd_outhex4(90, FH, g_model.mGvars[0][3] ) ;

//	lcd_outhex4(0,  2*FH, g_model.mGvars[1][0] ) ;
//	lcd_outhex4(30, 2*FH, g_model.mGvars[1][1] ) ;
//	lcd_outhex4(60, 2*FH, g_model.mGvars[1][2] ) ;
//	lcd_outhex4(90, 2*FH, g_model.mGvars[1][3] ) ;
...
// Script memory debug
//extern unsigned char *EndOfHeap ;
//	lcd_outhex4( 0, 2*FH, (uint32_t)EndOfHeap >> 16 ) ;
//	lcd_outhex4( 24, 2*FH, (uint32_t)EndOfHeap ) ;
//extern unsigned char *heap ;
//	lcd_outhex4( 0, 3*FH, (uint32_t)heap >> 16 ) ;
//	lcd_outhex4( 24, 3*FH, (uint32_t)heap ) ;
//	uint32_t space = EndOfHeap - heap ;
//	lcd_outhex4( 0, 4*FH, space >> 16 ) ;
//	lcd_outhex4( 24, 4*FH, space ) ;
//uint32_t getBasicAddress() ;
//	space = getBasicAddress() ;
//	lcd_outhex4( 60, 4*FH, space >> 16 ) ;
//	lcd_outhex4( 84, 4*FH, space ) ;
...
#else	
//	lcd_outhex4( 0,  1*FH, TelemetryData[FR_VCC     ] ) ;
//  lcd_outhex4( 30, 1*FH, TelemetryData[FR_CURRENT ] ) ;
//  lcd_outhex4( 60, 1*FH, TelemetryData[FR_ALT_BARO] ) ;
//  lcd_outhex4( 90, 1*FH, TelemetryData[FR_BASEMODE] ) ;

//  lcd_outhex4( 0,  2*FH, TelemetryData[FR_WP_DIST ] ) ;
//  lcd_outhex4( 30, 2*FH, TelemetryData[FR_HEALTH  ] ) ;
//  lcd_outhex4( 60, 2*FH, TelemetryData[FR_MSG     ] ) ;
//  lcd_outhex4( 90, 2*FH, TelemetryData[FR_HOME_DIR] ) ;
#endif

#ifndef PCB9XT
//  lcd_outhex4( 0,  3*FH, TelemetryData[FR_HOME_DIST] ) ;
//  lcd_outhex4( 30, 3*FH, TelemetryData[FR_CPU_LOAD ] ) ;
//  lcd_outhex4( 60, 3*FH, TelemetryData[FR_GPS_HDOP ] ) ;
//  lcd_outhex4( 90, 3*FH, TelemetryData[FR_WP_NUM   ] ) ;

//  lcd_outhex4( 0,  4*FH, TelemetryData[FR_WP_BEARING] ) ;
//  lcd_outhex4( 30, 4*FH, TelemetryData[FR_VOLTS     ] ) ;
//  lcd_outhex4( 60, 4*FH, TelemetryData[FR_TEMP2     ] ) ;
//  lcd_outhex4( 90, 4*FH, TelemetryData[FR_FUEL      ] ) ;
#endif
...
#ifndef PCB9XT
//	lcd_outhex4( 0,  5*FH, TelemetryData[FR_TEMP1     ] ) ;
//  lcd_outhex4( 30, 5*FH, TelemetryData[FR_A1_COPY   ] ) ;
...
void displayKeys( uint8_t x, uint8_t y, char *text, uint8_t count )
{
	while ( count )
	{
		char c = *text++ ;
#ifdef TOUCH
		lcd_putcAtt( x+4, y, c, DBLSIZE|CONDENSED ) ;
...
void menuProcAlpha(uint8_t event)
{
	...
	lcd_puts_P( 5*FW, 0, Palpha->PalphaHeading ? (char *)Palpha->PalphaHeading : PSTR( STR_NAME ) ) ;
	static MState2 mstate2 ;
#ifdef TOUCH
	TlExitIcon = 1 ;
#endif
	...
	lcd_putsAtt( 0, 12*FH+4, XPSTR("CAP SPACE DEL INS  <  >"), DBLSIZE|CONDENSED ) ;
#else	 
...
void menuTouch(uint8_t event)
{
  ...
  lcd_outhex4( 0, 3*FH, TouchX ) ;
  lcd_outhex4( 0, 4*FH, TouchY ) ;
...
//	lcd_outdezAtt( 10*FW, 2*FH, getTrimFlightPhase( 1, 0 ), 0 ) ;
////	lcd_outdezAtt( 15*FW, 2*FH, getTrimFlightPhaseOld( 1, 0 ), 0 ) ;
//	t_trim v = getRawTrimComplete( 1, 0 ) ;
//	lcd_outdezAtt( 10*FW, 3*FH, v.mode, 0 ) ;
//	lcd_outdezAtt( 10*FW, 4*FH, v.value, 0 ) ;
...
//		v = p->trim[0] ;
//	lcd_outdezAtt( 15*FW, 3*FH, v.mode, 0 ) ;
//	lcd_outdezAtt( 15*FW, 4*FH, v.value, 0 ) ;
////	v = rawTrimFix( 1, v ) ;
////	lcd_outdezAtt( 22*FW, 3*FH, v.mode, 0 ) ;
////	lcd_outdezAtt( 22*FW, 4*FH, v.value, 0 ) ;
...
//	lcd_outdezAtt( 22*FW, 2*FH, SetPhase, 0 ) ;
//	lcd_outdezAtt( 22*FW, 3*FH, SetIdx, 0 ) ;
//	lcd_outdezAtt( 22*FW, 4*FH, SetTrim, 0 ) ;

//	lcd_outdezAtt( 27*FW, 2*FH, SetP, 0 ) ;
//	lcd_outdezAtt( 27*FW, 3*FH, SetGet, 0 ) ;
    ...
    putsTmrMode(x+7*FW-FW/2 + X12SMALL_OFFSET,FH*3,0, 0, 0 ) ;
		...
		if ( i )
		{
  		PhaseData *p ;
			...
			if ( p->name[0] != ' ' )
  	 	{
				lcd_putsnAtt( 15*FW + X12SMALL_OFFSET-4*FH, 2*FH+4, p->name, 6, /*BSS*/ 0 ) ;
			}
			else
			{
		...
		if ( i )
		{
  		PhaseData *p ;
			...
			if ( p->name[0] != ' ' )
			{
				lcd_putsnAtt( 6*FW+2 + X12SMALL_OFFSET, 2*FH, p->name, 6, /*BSS*/ 0 ) ;
			}
			else
			{
...
//		displayTrims( 0 ) ;
...
#endif
	{
  	...
  	PhaseData *p ;
		...
		if ( i && p->name[0] )
		{
			lcd_putsnAtt( 2*FW + X12SMALL_OFFSET, 0, p->name, 6, /*BSS*/ 0 ) ;
		}
		else
		{
...
//                            blink = (alarmRaised[i] ? INVERS : 0);
                    ...
                    if ( g_model.frsky.channels[i].units == 3 )		// Current (A)
										{
                      lcd_outdezAtt(x0+FW, 4*FH,  TelemetryData[FR_A1_MAH+i], 0);
										}
										else
										{
...
//                if (frskyUsrStreaming)
...
#if defined(PCBX12D) || defined(PCBX10)
               	lcd_outdezAtt(3 * FW, 5*FH, TelemetryData[FR_RXRSI_COPY], DBLSIZE|CONDENSED|LEFT);
#else								 
               	lcd_outdezAtt(3 * FW - 2, 5*FH, TelemetryData[FR_RXRSI_COPY], DBLSIZE|LEFT);
#endif                
#ifdef ACCESS
 ...
 #if defined(PCBX12D) || defined(PCBX10)
                	lcd_outdezAtt(14 * FW, 5*FH, TelemetryData[FR_TXRSI_COPY], DBLSIZE|CONDENSED|LEFT);
 #else								 
                	lcd_outdezAtt(14 * FW - 4, 5*FH, TelemetryData[FR_TXRSI_COPY], DBLSIZE|LEFT);
 #endif                
								}
#else								 
								putTxSwr( 11 * FW - 2, 6*FH, 0 ) ;
 ...
 #if defined(PCBX12D) || defined(PCBX10)
                  lcd_outdezNAtt(13*FW, 1*FH, (uint16_t)TelemetryData[FR_RPM], DBLSIZE|CONDENSED|LEFT, 5);
#else								 
                  lcd_outdezNAtt(13*FW, 1*FH, (uint16_t)TelemetryData[FR_RPM], DBLSIZE|LEFT, 5);
#endif                
...
//                    blink = (alarmRaised[1] ? INVERS+BLINK : 0);
                ...
                lcd_outdezAtt(3 * FW, 7*FH, TelemetryData[FR_RXRSI_COPY], LEFT);
#ifdef ACCESS
								if (TelemetryDataValid[3] )
								{
									putTxSwr( 8 * FW , 7*FH, 0 ) ;
  	              lcd_outdezAtt(11 * FW, 7*FH, TelemetryData[FR_TXRSI_COPY], LEFT);
								}
#else								 
								putTxSwr( 8 * FW , 7*FH, 0 ) ;
                lcd_outdezAtt(11 * FW, 7*FH, TelemetryData[FR_TXRSI_COPY], LEFT);
...
//  							displayGPSformat( 4*FW, 1*FH, 0 ) ;
                lcd_puts_Pleft( 2*FH, PSTR(STR_LAT_EQ)) ;
								displayGPSdata( 3*FW, 2*FH, TelemetryData[FR_GPS_LAT], TelemetryData[FR_GPS_LATd], LEADING0 | blink, TelemetryData[FR_LAT_N_S] ) ;
								displayGPSdata( 3*FW, 3*FH, TelemetryData[FR_GPS_LONG], TelemetryData[FR_GPS_LONGd], LEADING0 | blink, TelemetryData[FR_LONG_E_W] ) ;
//								div_t qr ;
...
//                	lcd_putc(9*FW, 2*FH, '.') ;
//                	lcd_outdezNAtt(13*FW, 2*FH, TelemetryData[FR_GPS_LATd], LEADING0 | blink, -4);
//									qr = div( TelemetryData[FR_GPS_LONG], 100 ) ;
//                	lcd_outdezNAtt(6*FW, 3*FH, qr.quot, LEADING0 | blink, -3);
//                	lcd_outdezNAtt(9*FW, 3*FH, qr.rem, LEADING0 | blink, -2);
//                	lcd_putc(9*FW, 3*FH, '.') ;
//                	lcd_outdezNAtt(13*FW, 3*FH, TelemetryData[FR_GPS_LONGd], LEADING0 | blink, -4);
//								}
								uint16_t alt ;
                ...
                lcd_outdez(20*FW, 3*FH, TelemetryData[FR_COURSE] );
		  						...
		  						for (uint8_t k=0 ; k<totalCells ; k++)
									{
  									...
  									lcd_outdezNAtt( x, y, TelemetryData[FR_CELL1+k + vdisp], blink | PREC2, 4 ) ;
										x += 7*FW ;
                ...
                y0 = 7*FH;
            ...
            else// if ( tview == 0x50 )
						{
						 if ( ( g_model.telemetryProtocol == TELEMETRY_ARDUCOPTER ) || ( g_model.telemetryProtocol == TELEMETRY_ARDUPLANE ) || ( g_model.telemetryProtocol == TELEMETRY_MAVLINK ) )
						 {
								...
								// line 0, V, volt bat
 				        attr = PREC1 ;
 				        lcd_outdezAtt( 9 * FW+2, 0, TelemetryData[FR_VOLTS], attr ) ;
...
#define STR_MAV_HOME         "dth"
								...
								lcd_outdez( 21 * FW+1, 4*FH, TelemetryData[FR_HOME_DIST] ) ;
...
#define STR_MAV_CURRENT          "Cur"
                ...
                lcd_outdezAtt( 20 * FW+3, 5*FH, TelemetryData[FR_CURRENT], PREC1 ) ;
//		 line 6 left "Rssi"
...
// Mavlink - home direction and distance on graphic?								
              ...
              lcd_outdezAtt( 17*FW-1, 0*FH, TelemetryData[FR_RXRSI_COPY], 0 ) ;
              lcd_outdezAtt( 61, 1*FH, TelemetryData[FR_VOLTS], PREC1|DBLSIZE ) ;
//							lcd_outdezAtt( 125, 1*FH, convertRxv( TelemetryData[FR_RXV] ), PREC1|DBLSIZE ) ;
							lcd_outdezAtt( 125, 1*FH, TelemetryData[FR_RXV], PREC1|DBLSIZE ) ;
							lcd_outdezAtt( 61, 3*FH, TelemetryData[FR_CURRENT], PREC1|DBLSIZE ) ;
              lcd_outdezAtt( 125, 3*FH, TelemetryData[FR_TEMP1], DBLSIZE ) ;
              lcd_outdezAtt( 14*FW, 5*FH, TelemetryData[FR_RPM], DBLSIZE ) ;
...
#define RDISP_OFF_0			0
      ...
      if( sub==subN )
			...
			lcd_outdezAtt( PARAM_OFS+RDISP_OFF_0, y, g_eeGeneral.selectedTheme, (sub==subN) ? blink : 0 ) ;
			if( attr )
			{
...
uint8_t InputMap[130] ;
uint8_t InputMapSize ;

uint8_t unmapInputSource( uint8_t index ) ;

void displayInputSource( uint8_t x, uint8_t y, uint8_t index, uint8_t att )
{
	uint8_t value ;
	uint8_t swIdx = 0 ;
	value = InputMap[ index ] ;
...
{
	uint8_t action ;
	action = yesNoMenuExit( event, PSTR(STR_DELETE_MIX) ) ;
...
void menuInputsLimit(uint8_t event)
{
  ...
  lcd_puts_Pleft(2*FH, XPSTR("Max Inputs Reached"));
  lcd_outdezAtt(20*FW, 2*FH, getInputsCount(),0);
...
//			}
  ...
  for ( uint32_t k = 0 ; k<SCREEN_LINES-1 ; k += 1 )
  {
    ...
    switch(i)
      ...
      case 0:
			{	
				lcd_puts_P(  0,y,PSTR(STR_2SOURCE)+1) ;
				uint32_t value = pinput->srcRaw ;
				if ( value >= 128 )
				{
					putsChnOpRaw( FW*14,y, MIX_3POS, value-128, 0, attr ) ;
				}
				else
				{
					putsChnOpRaw( FW*14,y, value, 0, 0, attr ) ;
				}
...
//					}
	    ...
	    case 1 :
			{	
				lcd_puts_P(  0, y, PSTR(STR_2WEIGHT)+1);
...
//				lcd_outdezAtt( 15*FW, y, pinput->weight, attr ) ;
//				if (attr)
//				{
...
//				}
			break ;
	    case 2 :
        lcd_puts_P( 0,y,PSTR(STR_2SWITCH)+1);
        putsDrSwitches(11*FW, y, pinput->swtch,attr);
        if(attr) CHECK_INCDEC_MODELSWITCH( pinput->swtch, -MaxSwitchIndex, MaxSwitchIndex);
			break ;
	    case 3 :
				lcd_puts_P(  0,y,XPSTR("TRIM"));
				lcd_putsAttIdx( 12*FW, y, (char *)XPSTR("\003 OnOffRudEleThrAil"), pinput->carryTrim, attr ) ;
   			if (attr)
				{
	    ...
	    case 4 :
			{	
				...
				for ( uint32_t p = 0 ; p<MAX_MODES+2 ; p++ )
				{
   				...
   				lcd_putcAtt( (5+p)*(FW+1), y, '0'+p, ( z & b ) ? 0 : INVERS ) ;
					if( attr && ( g_posHorz == p ) )
					{
	    ...
	    case 5 :
			{	
				lcd_puts_P(  0,y,PSTR(STR_OFFSET));
...
//				lcd_outdezAtt( 15*FW, y, pinput->offset, attr ) ;
//   			if (attr)
//				{
...
//				}
	    ...
	    case 6 :
			{	
				lcd_putsAtt(  0, y, (char *)XPSTR("Curve"), 0 ) ;
				lcd_putsAttIdx( 9*FW, y, (char *)XPSTR("\005 None FuncCurve Diff Expo"), pinput->mode, attr ) ;
			...
			break ;
	    case 7 :
				lcd_putsAtt(  0, y, (char *)XPSTR("Value"), 0 ) ;
...
#endif
						{
							lcd_outdezAtt(15*FW,y,pinput->curve,attr);
...
//									}
			...
			break ;
	    case 8 :
				lcd_puts_P(  0,y,XPSTR("Side"));
				lcd_putsAttIdx( 12*FW, y, (char *)XPSTR("\003---x>0x<0"), pinput->side, attr ) ;
   			if (attr)
				{
  ...
  if( value && (value <= 4) )
	{
		...
		if ( !( *ptr && (*ptr != ' ' ) ) )
		{
    	lcd_putsAttIdx( x+FW, y, PSTR(STR_STICK_NAMES), value-1, attr ) ;
		}
		else
		{
			lcd_putsAtt( x+FW, y, ptr, attr ) ;
		}
	}
	else
	{
		lcd_outdezNAtt(x+3*FW-1, y, inputIndex, attr + LEADING0, 2) ;
	}
}
  ...
  for ( uint32_t chan = 1 ; chan <= NUM_INPUTS ; chan += 1 )
    ...
    if ( t_pgOfs <= current && current-t_pgOfs < SCREEN_LINES-1)
    	...
    	uint8_t attr = 0 ;
			...
			displayInputName( 1, (current-t_pgOfs+1)*FH, chan, attr ) ;

//			lcd_img( 1, (current-t_pgOfs+1)*FH, IconInput, 0, 0 ) ;
//			lcd_outdezNAtt(3*FW, (current-t_pgOfs+1)*FH, chan, attr + LEADING0, 2) ;
    }
    	...
    	do
			{
				if (t_pgOfs <= current )
				{
					if ( current-t_pgOfs < SCREEN_LINES-1 )
					{
    	  	  uint16_t y = (current-t_pgOfs+1)*FH ;
    				uint8_t attr = 0 ;
...
//						}
						...
						if ( g_model.vars )
						{
							if ( pinput->varForWeight )
							{
								displayVarName( (12-6)*FW, y, pinput->weight, attr ) ;
							}
							else
							{
								lcd_outdezAtt( 12*FW, y, pinput->weight, attr ) ;
							}
						}
						else
#endif
...
//#endif
								...
								{
									temp = -temp - 1 ;
  								lcd_putcAtt(8*FW-4*FW, y,'-',attr) ;
								}
								dispGvar( 12*FW-3*FW, y, temp+1, attr ) ;
							}
							else
							{
								lcd_outdezAtt( 12*FW, y, temp, attr ) ;
							}
						}
