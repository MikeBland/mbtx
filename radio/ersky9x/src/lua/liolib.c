...
static int read_chars (lua_State *L, FIL *f, size_t n) {
  ...
  if (f_read(f, (BYTE *)p, n, &nr) != FR_OK) nr = 0; /* try to read 'n' chars */
  luaL_addsize(&b, nr);
  luaL_pushresult(&b);  /* close buffer */
...
static int g_write (lua_State *L, FIL *f, int arg) {
  ...
  for (; nargs--; arg++) {
    if (lua_type(L, arg) == LUA_TNUMBER) {
      ...
      UINT w;
      status = status && (f_write(f, (BYTE *)buff, len, &w) == FR_OK && w == len);
    }
    else {
      size_t l; UINT w;
      const char *s = luaL_checklstring(L, arg, &l);
      status = status && (f_write(f, (BYTE *)s, l, &w) == FR_OK && w == l);
    }
  }
...
static int io_seek (lua_State *L) {
  FIL *f = tofile(L);
  lua_Unsigned offset = luaL_checkinteger(L, 2);
