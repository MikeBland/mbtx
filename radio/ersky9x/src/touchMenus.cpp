...
//		lcdDrawSizedText( x, y, PSTR(STR_CH), 2, 0, fcolour ) ;
////  	PUTS_NUM_N( x1, y, idx1, 0, 2, fcolour, bcolour ) ;
//		lcdDrawNumber( LcdNextPos, y, idx1, LEFT, 0, fcolour ) ;
...
// 	putsTime( TRIGHT-TRMARGIN-2*FW, y+TVOFF/2, ptm->tmrVal, 0, 0 ) ;
//	restoreEditColours() ;
...
//	}
//	saveEditColours( sub == subN, colour ) ;
//  putsTmrMode(TRIGHT-TRMARGIN-4*FW,y+TVOFF/2,0, timer, 1 ) ;
//	restoreEditColours() ;
//	y += TFH ;
//	subN += 1 ;
...
//	ptm->tmrModeB = edit_dr_switch( TRIGHT-TRMARGIN-4*FW, y+TVOFF/2, ptm->tmrModeB, LUA_RIGHT, doedit, event ) ;
//	restoreEditColours() ;
//	y += TFH ;
//	subN += 1 ;
...
//	saveEditColours( sub == subN, colour ) ;
//	sw = edit_dr_switch( TRIGHT-TRMARGIN-4*FW, y+TVOFF/2, sw, 0, doedit, event ) ;
//	restoreEditColours() ;
//	if ( timer == 0 )
//	{
...
//	lcdDrawText( 4*FW, 0, PSTR(STR_LIMITS), INVERS|BOLD ) ;
...
//  	PUTS_ATT_COLOUR( 3*FW, y+TVOFF/2, PSTR(STR_COPY_TRIM), 0, attr ? ~LcdForeground : LcdForeground ) ; // , attr ? ~LcdBackground : LcdBackground ) ;
//		lcd_hline( 0, y+TFH, TRIGHT ) ;
//	}
//}
...
//					LcdForeground = ~LcdForeground ;
//				}
//				phase->swtch = edit_dr_switch( TRIGHT-TRMARGIN, y+TVOFF/2, phase->swtch, LUA_RIGHT, attr ? EDIT_DR_SWITCH_EDIT : 0, event ) ;
//				LcdBackground = oldBcolour ;
//				LcdForeground = oldFcolour ;
//			}
//			break ;
...
//					LcdForeground = ~LcdForeground ;
//				}
//				phase->swtch2 = edit_dr_switch( TRIGHT-TRMARGIN, y+TVOFF/2, phase->swtch2, LUA_RIGHT, attr ? EDIT_DR_SWITCH_EDIT : 0, event ) ;
//				LcdBackground = oldBcolour ;
//				LcdForeground = oldFcolour ;
//			}
//			break ;
...
//					LcdForeground = ~LcdForeground ;
//				}
//        putsTrimMode( TRIGHT-2*FW, y+TVOFF/2, s_currIdx, k-2, 0 ) ;
...
//				LcdBackground = attr ? ~colour : colour ;
//				alphaEditName( TRIGHT-sizeof(phase->name)*FW-FW, y+TVOFF/2, (uint8_t *)phase->name, sizeof(phase->name), attr|ALPHA_NO_NAME, (uint8_t *)0 ) ;
//				LcdBackground = oldBcolour ;
//			}
//			break ;
...
//		saveEditColours( attr, colour ) ;
//		alphaEditName( TRIGHT-10*FW-5, y+TVOFF/2, (uint8_t *)g_model.name, sizeof(g_model.name), attr|ALPHA_NO_NAME, (uint8_t *)0 ) ;
//		restoreEditColours() ;
...
//		{
//		  PUTC( 2*FW+i*(2*FW+3), y+TVOFF/2, 'A'+i ) ;
//			PUTC( 3*FW+i*(2*FW+3), y+TVOFF/2, PSTR(HW_SWITCHARROW_STR)[states & 0x03] ) ;
//		  states >>= 2 ;
//		}
//		PUTC( 2*FW+7*(2*FW+3), y+TVOFF/2, '6' ) ;
//		PUTC( 3*FW+7*(2*FW+3), y+TVOFF/2, 'P' ) ;
//		PUTC( 4*FW+7*(2*FW+3), y+TVOFF/2, ( (states >> 2) & 0x07) + '0' ) ;
//		if( sub == subN )
//		{
...
//					saveEditColours( attr, colour ) ;
//					putsChnRaw( TRIGHT-TRMARGIN-4*FW, y, pvad->source, LUA_RIGHT ) ;
//					restoreEditColours() ;
//					if ( attr )
//					{
...
//					saveEditColours( attr, colour ) ;
//					displayVoiceRate( TRIGHT-TRMARGIN, y+TVOFF/2, pvad->rate, LUA_RIGHT ) ;
//					restoreEditColours() ;
//	    		if(attr)
//					{
...
//					saveEditColours( attr, colour ) ;
//  				PUTS_NUM(TRIGHT-TRMARGIN,y+TVOFF/2, pvad->delay, PREC1 ) ;
//					restoreEditColours() ;
//	    		if(attr)
//					{
...
//					saveEditColours( attr, colour ) ;
//					PUTS_ATT( TRIGHT-TRMARGIN-9*FW, y+TVOFF/2, XPSTR("MENU LONG"), 0 ) ;
//					restoreEditColours() ;
//  				if( attr )
//					{
//						if ( checkForMenuEncoderLong( event ) )
...
//		if ( attr )
//		{
//			lcdDrawSolidFilledRectDMA( 0, (y)*TSCALE, TRIGHT*TSCALE, TFH*TSCALE-2, ~LcdBackground ) ;
...
//		LcdBackground = oldBcolour ;
//		LcdForeground = oldFcolour ;
//		lcd_hline( 0, y+TFH-TVOFF/2, TRIGHT ) ;
//  }
...
//	  	PUTC( THOFF, TTOP+TVOFF/2, 'F' ) ;
//  		PUTC( THOFF+FW, TTOP+TVOFF/2, '0' ) ;
//			PUTS_ATT( TRIGHT-TRMARGIN-10*FW, TTOP+TVOFF/2, XPSTR("R   E   T   A"), attr ) ;
...
//	{
//		attr = sub==subN ? INVERS : 0 ;
//    PUTS_AT_IDX( THOFF/2, y+TVOFF/2, PSTR(STR_STICK_NAMES), i, 0 ) ;
...
//				putsDrSwitchesColour( TRIGHT-5*FW, y+TVOFF, g_eeGeneral.screenShotSw, LUA_RIGHT, attr ? ~LcdForeground : LcdForeground, attr ? ~colour : colour ) ;
...
//	attr = (sub==subN) ? INVERS : 0 ;
//	drawItem( (char *)PSTR(STR_LIGHT_SWITCH), y, attr ) ;
//	putsDrSwitchesColour( TRIGHT-TRMARGIN, y+TVOFF/2, g_eeGeneral.lightSw, LUA_RIGHT, attr ? ~LcdForeground : LcdForeground, attr ? ~colour : colour ) ;
//	if(attr)
//	{
...
//    else
//		{
//  		PUTS_ATT_COLOUR( TRIGHT-5-3*FW, y+TVOFF, PSTR(STR_OFF), 0, attr ? ~LcdForeground : LcdForeground ) ; //, attr ? ~colour : colour ) ;
//		}
//    if ( attr )
//		{
...
//	}
//  putsChnRaw( TRIGHT-TMARGIN, y+TVOFF/2, g_model.swashCollectiveSource, LUA_RIGHT, attr ? ~LcdForeground : LcdForeground, attr ? ~colour : colour ) ;
//  y += TFH ;
//	subN += 1 ;
...
//{
//  PUTS_NUM( TRIGHT-TRMARGIN, y+TVOFF/2, val+1, 0 ) ;
...
//				}
//				saveEditColours( attr, colour ) ;
//				putsChnRaw( TRIGHT-TRMARGIN, y+TVOFF/2, x, LUA_RIGHT ) ;
//				restoreEditColours() ;
//				if( attr )
//				{
//					x = mapPots( x ) ;
...
//				saveEditColours( attr, colour ) ;
//  			PUTC_ATT( TRIGHT-TRMARGIN-FW, y+TVOFF/2, pscaler->neg ? '-' : '+', 0 ) ;
//				restoreEditColours() ;
...
//				saveEditColours( attr, colour ) ;
//				PUTS_NUM( TRIGHT-TRMARGIN, y+TVOFF/2, pscaler->precision, 0 ) ;
//				restoreEditColours() ;
...
//  			PUTS_AT_IDX( THOFF/2, y+TVOFF/2, XPSTR("\012Chans 1-8 Chans 9-16"), pModule->highChannels, 0 ) ;
//				if ( attr )
//				{
...
//		  	PUTS_AT_IDX( THOFF/2, y+TVOFF/2, XPSTR("\014Telemetry   No Telemetry"), pModule->disableTelemetry, 0 ) ;
//				if ( attr )
//				{
...
//			LcdForeground = ~LcdForeground ;
//		}
//    PUTS_ATT( THOFF+FW*0, y+TVOFF/2, PSTR(STR_CV), 0 ) ;
//    PUTS_NUM( (k<9) ? THOFF+FW*3-1 : THOFF+FW*4-2, y+TVOFF/2, k+1, 0 ) ;
//		LcdBackground = oldBcolour ;
//		LcdForeground = oldFcolour ;
...
//		LcdBackground = ~LcdBackground ;
//	}
//	PUTS_ATT( THOFF, y+TVOFF/2, PSTR(STR_Trainer), 0 ) ;
//	LcdForeground = oldFcolour ;
//	LcdBackground = oldBcolour ;
...
//			LcdBackground = ~LcdBackground ;
//		}
//		displayModuleName( THOFF, y+TVOFF/2, i, 0 ) ;
//		displayProtocol( THOFF+10*FW, y+TVOFF/2, g_model.Module[i].protocol, 0 ) ;
//		LcdForeground = oldFcolour ;
//		LcdBackground = oldBcolour ;
//		lcd_hline( 0, y+TFH, TRIGHT ) ;
//  	y += TFH ;
...
//				LcdBackground = ~LcdBackground ;
//			}	
//			PUTS_P( THOFF, y+TVOFF/2, PSTR(STR_RANGE) ) ;
//			displayModuleName( 12*FW+THOFF, y+TVOFF/2, i > 1, 0 ) ;
//			LcdForeground = oldFcolour ;
//			LcdBackground = oldBcolour ;
//  		if(sub==subN)
//			{
...
//	uint8_t doedit = attr ? EDIT_DR_SWITCH_MOMENT | EDIT_DR_SWITCH_EDIT : EDIT_DR_SWITCH_MOMENT ;
//	saveEditColours( attr, colour ) ;
//	g_model.musicData.musicStartSwitch = edit_dr_switch( TRIGHT-TRMARGIN-4*FW, y+TVOFF/2, g_model.musicData.musicStartSwitch, 0, doedit, event ) ;
//	restoreEditColours() ;
//	y += TFH ;
//	subN += 1 ;
...
//	doedit = attr ? EDIT_DR_SWITCH_MOMENT | EDIT_DR_SWITCH_EDIT : EDIT_DR_SWITCH_MOMENT ;
//	saveEditColours( attr, colour ) ;
//	g_model.musicData.musicPauseSwitch = edit_dr_switch( TRIGHT-TRMARGIN-4*FW, y+TVOFF/2, g_model.musicData.musicPauseSwitch, 0, doedit, event ) ;
//	restoreEditColours() ;
//	y += TFH ;
//	subN += 1 ;
...
//	doedit = attr ? EDIT_DR_SWITCH_MOMENT | EDIT_DR_SWITCH_EDIT : EDIT_DR_SWITCH_MOMENT ;
//	saveEditColours( attr, colour ) ;
//	g_model.musicData.musicPrevSwitch = edit_dr_switch( TRIGHT-TRMARGIN-4*FW, y+TVOFF/2, g_model.musicData.musicPrevSwitch, 0, doedit, event ) ;
//	restoreEditColours() ;
//	y += TFH ;
//	subN += 1 ;
...
//	doedit = attr ? EDIT_DR_SWITCH_MOMENT | EDIT_DR_SWITCH_EDIT : EDIT_DR_SWITCH_MOMENT ;
//	saveEditColours( attr, colour ) ;
//	g_model.musicData.musicNextSwitch = edit_dr_switch( TRIGHT-TRMARGIN-4*FW, y+TVOFF/2, g_model.musicData.musicNextSwitch, 0, doedit, event ) ;
//	restoreEditColours() ;
...
//	PUTS_ATT_COLOUR( TRIGHT-TRMARGIN-MUSIC_NAME_LENGTH*FW, y+TVOFF/2, (char *)g_eeGeneral.musicVoiceFileName, 0, attr ? ~LcdForeground : LcdForeground ) ; //, attr ? ~colour : colour ) ;
//	if (attr)
//	{
...
//	if ( MusicPlaying == MUSIC_STOPPED )
//	{
//		PUTS_ATT_COLOUR( TRIGHT-TRMARGIN-5*FW, y+TVOFF/2, (char *)XPSTR("Start"), 0, attr ? ~LcdForeground : LcdForeground) ; //, attr ? ~colour : colour ) ;
//	}
//	else
//	{
//		PUTS_ATT_COLOUR( TRIGHT-TRMARGIN-4*FW, y+TVOFF/2, (char *)XPSTR("Stop"), 0, attr ? ~LcdForeground : LcdForeground) ; //, attr ? ~colour : colour ) ;
//		y += TFH ;
//		subN += 1 ;
...
//		PUTS_ATT_COLOUR( TRIGHT-TRMARGIN, y+TVOFF/2, (char *)(MusicPlaying == MUSIC_PAUSED ? XPSTR("Resume") : XPSTR(" Pause")), LUA_RIGHT, attr ? ~LcdForeground : LcdForeground ) ; //, attr ? ~colour : colour ) ;
//		if ( g_eeGeneral.musicType )
//		{
//			if ( MusicPlaying == MUSIC_PLAYING )
//			{
...
//	{
//		PUTS_P( THOFF, TTOP+2*TFH+TVOFF/2, XPSTR("val") ) ;
//	}
//	else
//	{
//		PUTS_P( THOFF, TTOP+2*TFH+TVOFF/2, XPSTR("V2") ) ;
//	}
...
//  {
//		saveEditColours( sub==1, colour ) ;
//   	putsDrSwitches(TRIGHT-TRMARGIN, TTOP+TFH+TVOFF/2, cs.v1, LUA_RIGHT ) ;
//		restoreEditColours() ;
//		saveEditColours( sub==2, colour ) ;
//   	putsDrSwitches(TRIGHT-TRMARGIN, TTOP+2*TFH+TVOFF/2, cs.v2, LUA_RIGHT ) ;
//		restoreEditColours() ;
//  }
...
//			att = PREC1 ;
//		}
//	  PUTS_P( THOFF,TTOP+TFH+TVOFF/2, XPSTR("Off") ) ;
...
//			att = PREC1 ;
//		}
//	  PUTS_P( THOFF,TTOP+2*TFH+TVOFF/2, XPSTR("On ") ) ;
...
//			att = PREC1 ;
//		}
//		PUTS_P( THOFF, TTOP+2*TFH+TVOFF/2, XPSTR("Time") ) ;
//		drawNumber( TRIGHT-TRMARGIN, TTOP+2*TFH, x+1, att) ; //, (sub == 2) ? ~colour : colour ) ;
//		cs.exfunc = 0 ;
//	}
//	else// cstate == U16
...
//		x |= cs.bitAndV3 << 8 ;
//		saveEditColours( sub==1, colour ) ;
//		putsChnRaw( TRIGHT-TRMARGIN, TTOP+TFH+TVOFF, cs.v1u, LUA_RIGHT ) ;
//		restoreEditColours() ;
...
//	PUTS_AT_IDX( 7*FW+THOFF, TTOP+3*TFH+TVOFF+Vextra, XPSTR("\003ANDOR XOR"), cs.exfunc, 0 ) ;
...
////		if ( k < NUM_SKYCSW )
////		{
//			PUTS_AT_IDX( 2*FW+1, y+TVOFF/2, PSTR(CSWITCH_STR),cs.func, 0) ;

//  	  uint8_t cstate = CS_STATE(cs.func);

//	  	if( (cstate == CS_VOFS) || (cstate == CS_2VAL) )
//    	{
//				putsChnRaw(    10*FW-6, y+TVOFF/2, cs.v1u  , 0);
//	  	  if ( ( (cs.v1u > CHOUT_BASE+NUM_SKYCHNOUT) && ( cs.v1u < EXTRA_POTS_START ) ) || ( cs.v1u >= EXTRA_POTS_START + 8) )
// 				{
//					int16_t value = convertTelemConstant( cs.v1u-CHOUT_BASE-NUM_SKYCHNOUT-1, cs.v2 ) ;
...
//    	{
//    	  putsChnRaw(    10*FW-6, y+TVOFF/2, cs.v1u  , 0);
//    	  putsChnRaw(    14*FW-4, y+TVOFF/2, cs.v2u  , 0);
//    	}
//			else if(cstate == CS_TIMER)
//			{
//				int8_t x ;
...
//					att = PREC1 ;
//				}
//    	  PUTS_NUM( 17*FW-2, y+TVOFF/2, x+1 , att ) ;
//			}
//			else// cstate == U16
//			{
//				uint16_t x ;
//				x = cs.v2u ;
//				x |= cs.bitAndV3 << 8 ;
//    	  putsChnRaw( 10*FW-6-FW, y+TVOFF/2, cs.v1  , 0);
...
////						{
////							PUTS_AT_IDX( 6*FW, y, PSTR(STR_V_OPT1), sd->opt.ss.swtch-MAX_SKYDRSWITCH-1, attr ) ;
////						}
////						else
//						{
...
////  	 		else if (j == 1)
////  	 		{
////					PUTS_AT_IDX( 10*FW, y, PSTR("\005-----variovarA2"), Function[0].func, attr ) ;
////  	 		  if(active)
////					{
...
//				drawItem( (char *)XPSTR("Function"), y, attr ) ;
//				drawIdxText( y*2+TVOFF/2, (char *)StringAdjustFunctions, pgvaradj->function, attr ) ;
// 				if(attr)
//				{ 
...
//				drawItem( (char *)PSTR(STR_SWITCH), y, attr ) ;
//				saveEditColours( attr, colour ) ;
//				pgvaradj->swtch = edit_dr_switch( TRIGHT-TRMARGIN, y+TVOFF/2, pgvaradj->swtch, LUA_RIGHT, attr ? EDIT_DR_SWITCH_EDIT : 0, event ) ;
//				restoreEditColours() ;
//			break ;
//			case 3 : // Param
//				if ( ( pgvaradj->function < 4 ) || ( pgvaradj->function > 6 ) )
...
//					drawItem( (char *)PSTR(STR_SWITCH), y, attr ) ;
//					saveEditColours( attr, colour ) ;
//					pgvaradj->switch_value = edit_dr_switch( TRIGHT-TRMARGIN, y+TVOFF/2, pgvaradj->switch_value, LUA_RIGHT, attr ? EDIT_DR_SWITCH_EDIT : 0, event ) ;
//					restoreEditColours() ;
//				}
//			break ;
//		}
...
//		if ( k < 9 )
//		{
//  		PUTC( 1*FW, y+TVOFF/2, k+'1' ) ;
//		}
//		else
//		{
//  		PUTC( 1*FW, y+TVOFF/2, k > 18 ? '2' : '1' ) ;
//  		PUTC( 2*FW, y+TVOFF/2, ( (k+1) % 10) +'0' ) ;
//		}
...
////#else
//					PUTS_AT_IDX( 3*FW+2, y+TVOFF/2, PSTR(STR_GV_SOURCE), idx-6, attr ) ;
////#endif
//				}
//				else
//				{
//  				PUTS_P( 3*FW+2, y+TVOFF/2, XPSTR("GV") ) ;
//					PUTC_ATT( 5*FW+2, y+TVOFF/2, idx+'1', 0 ) ;
//				}
////			}
////			else if ( j == 1 )
////			{
//				PUTS_AT_IDX( 7*FW, y+TVOFF/2, StringAdjustFunctions, pgvaradj->function, 0);
////			}
////			else if ( j == 2 )
////			{
//      	putsDrSwitches(13*FW-1, y+TVOFF/2, pgvaradj->swtch, 0);
////			}
////			else
////			{
//				if ( ( pgvaradj->function < 4 ) || ( pgvaradj->function > 6 ) )
...
//					if ( pgvaradj->function == 3 )
//					{
//						PUTS_AT_IDX( 18*FW, y+TVOFF/2, PSTR(STR_GV_SOURCE), pgvaradj->switch_value, attr ) ;
//					}
//					else
//					{
//						PUTS_NUM( 21*FW, y+TVOFF/2, pgvaradj->switch_value, 0 ) ;
//					}
//				}
//				else
//				{
//      		putsDrSwitches(17*FW, y+TVOFF/2, pgvaradj->switch_value, 0);
//				}
////			}
////		}
////		LcdBackground = oldBcolour ;
...
//					drawItem( (char *)PSTR(STR_SWITCH), y, attr ) ;
//					saveEditColours( attr, colour ) ;
//					g_model.gvswitch[s_currIdx] = edit_dr_switch( TRIGHT-TRMARGIN, y+TVOFF/2, g_model.gvswitch[s_currIdx], LUA_RIGHT, attr ? EDIT_DR_SWITCH_EDIT : 0, event ) ;
//					restoreEditColours() ;
//				}
//			break ;
//			case 1 :
...
//		}
//		saveEditColours( attr, LcdBackground ) ;
//  	PUTS_P( THOFF, y+TVOFF/2, PSTR(STR_GV));
//		PUTC( THOFF+2*FW, y+TVOFF/2, k+ ((k > 8) ? '8' : '1') ) ;
//		GvarData *pgvar ;
////#if MULTI_GVARS
////		if ( g_model.flightModeGvars )
////		{
...
////#endif
//		{
//	    putsDrSwitches( 7*FW, y+TVOFF/2, g_model.gvswitch[k], 0 );
//			uint32_t t ;
//			uint8_t *psource ;
////			pgvar = &g_model.gvars[k] ;
//			psource = &pgvar->gvsource ;
//			t = *psource ;
//			PUTS_AT_IDX( 14*FW-4, y+TVOFF/2, PSTR(STR_GV_SOURCE), t, 0 ) ;
//			PUTS_NUM( TRIGHT-TRMARGIN, y+TVOFF/2, pgvar->gvar, 0 ) ;
//		}
////#if MULTI_GVARS
////		else
////		{
...
//void putsChnOpRaw( uint8_t x, uint8_t y, uint8_t source, uint8_t switchSource, uint8_t output, LcdFlags attr ) ;
...
//				putsChnOpRaw( TRIGHT-TRMARGIN, y+TVOFF/2, value, md2->switchSource, md2->disableExpoDr, LUA_RIGHT ) ;
//				restoreEditColours() ;
//					if ( attr )
//					{
...
//					{
//						uint16_t result = extendedValueEdit( md2->weight, md2->extWeight, attr, y+TVOFF/2, event, TRIGHT-TRMARGIN ) ;
...
//#endif
//				{
//					uint16_t result = extendedValueEdit( md2->sOffset, md2->extOffset, attr, y+TVOFF/2, event, TRIGHT-TRMARGIN ) ;
//					md2->sOffset = result & 0x00FF ;
//					md2->extOffset = result >> 8 ;
//				}
//				restoreEditColours() ;
...
//				drawIdxText( y*2, (char *)XPSTR("\005 None FuncCurve Diff Expo"), curveFunction, attr ) ;
...
//							value += 1000 ;
//						}
//						value = editVarCapableValue( TRIGHT-TRMARGIN, y+TVOFF/2, value, -350, 350, NUM_VARS, attr, event ) ;
//						if ( value > 900 )
//						{
//							value -= 1000 ;
//							diffIsGvar = 1 ;
...
//							value += 510 ;
//						}	
//						value = gvarDiffValue( TRIGHT-TRMARGIN, y+TVOFF/2, value, attr|0x80000000, event ) ;
//						if ( value > 500 )
//						{
//							diffIsGvar = 1 ;
//							value -= 510 ;
...
//								curveValue += 950 ;
//							}
//							curveValue = editVarCapableValue( TRIGHT-TRMARGIN, y+TVOFF/2, curveValue, 0, 100, NUM_VARS, attr, event ) ;
//							if ( curveValue > 800 )
//							{
//								curveValue -= 950 ;
//								md2->varForExpo = 1 ;
...
//#endif
//						{
//          	  PUTS_NUM(TRIGHT-TRMARGIN,y+TVOFF/2,curveValue,0);
...
//							}
//						}
//						put_curve( TRIGHT-TRMARGIN-3*FW, y+TVOFF/2, temp, 0 ) ;
//						if ( curveFunction )
//						{
//          	  if(attr)
//							{
...
//				saveEditColours( attr, DimBackColour ) ;
//				putsDrSwitches(TRIGHT-TRMARGIN-4*FW, y+TVOFF/2, md2->swtch, 0) ;
//				restoreEditColours() ;
...
//   				PUTC_ATT( TRIGHT-TRMARGIN-7*(FW+3)+p*(FW+2), y+TVOFF/2, '0'+p, ( z & b ) ? 0 : INVERS ) ;
//					if( attr && ( g_posHorz == p ) )
//					{
...
//				saveEditColours( attr, DimBackColour ) ;
//        if(b)
//          PUTS_NUM( TRIGHT-TRMARGIN, y+TVOFF/2, b, 0 ) ;
//        else
//          PUTS_ATT( TRIGHT-TRMARGIN-3*FW, y+TVOFF/2, PSTR(STR_OFF), 0 ) ;
//				restoreEditColours() ;
//        if(attr) { CHECK_INCDEC_H_MODELVAR_0( b, 3); md2->mixWarn = b ; }
//      break;
//      case 11:
//				drawItem( (char *)PSTR(STR_2MULTIPLEX)+1, y, attr ) ;
//				drawIdxText( y*2, (char *)PSTR(STR_ADD_MULT_REP), md2->mltpx, attr ) ;
...
//	}		 
//	saveEditColours( attr, DimBackColour ) ;
//	putsChnRaw( THOFF, TTOP+TVOFF/2,l_expoChan+1,0) ;
//	restoreEditColours() ;
...
//	{
//    case DR_MID:
//      PUTS_P( 8*FW, TTOP+TVOFF/2,PSTR(STR_4DR_MID)+1);
//    break;
//    case DR_LOW:
//      PUTS_P( 8*FW, TTOP+TVOFF/2,PSTR(STR_4DR_LOW)+1);
//    break;
//    default: // DR_HIGH:
//      PUTS_P( 8*FW, TTOP+TVOFF/2,PSTR(STR_4DR_HI)+1);
//    break;
//	}
...
//	PUTS_P( THOFF,TTOP+TFH+TVOFF/2,PSTR(STR_2EXPO)+1);
//	restoreEditColours() ;
...
//	editExpoVals( event, attr, 17*FW/2, TTOP+2*TFH+TVOFF/2, expoDrOn, DR_EXPO, DR_LEFT ) ;
//	restoreEditColours() ;
//	if ( stkVal == DR_BOTH )
//	{
//		if ( TouchAdjustValue )
...
//	editExpoVals( event, attr, 16*FW, TTOP+2*TFH+TVOFF/2, expoDrOn, DR_EXPO, DR_RIGHT ) ;
//	restoreEditColours() ;
...
//	}
//	saveEditColours( ( sub==subN ), DimBackColour ) ;
//	PUTS_P( THOFF,TTOP+3*TFH+TVOFF/2,PSTR(STR_2WEIGHT)+1);
//	restoreEditColours() ;
//	if ( attr )
//	{
...
//	editExpoVals( event, attr, 17*FW/2, TTOP+4*TFH+TVOFF/2, expoDrOn, DR_WEIGHT, DR_LEFT ) ;
//	restoreEditColours() ;
//	if ( stkVal == DR_BOTH )
//	{
//		if ( TouchAdjustValue )
...
//	editExpoVals( event, attr, 16*FW, TTOP+4*TFH+TVOFF/2, expoDrOn, DR_WEIGHT, DR_RIGHT ) ;
//	restoreEditColours() ;
//	lcd_hline( 0, TTOP+4*TFH, TMID-15 ) ;
//	subN += 1 ;
//	PUTS_P( THOFF, TTOP+5*TFH+TVOFF/2,PSTR(STR_DR_SW1));
...
//	} 
//	saveEditColours( attr, DimBackColour ) ;
//	editExpoVals( event, attr, 10*FW, TTOP+5*TFH+TVOFF/2, DR_DRSW1, 0, 0 ) ;
//	restoreEditColours() ;
//	subN += 1 ;
	
//	PUTS_P( THOFF, TTOP+6*TFH+TVOFF/2,PSTR(STR_DR_SW2));
...
//	} 
//	saveEditColours( attr, DimBackColour ) ;
//	editExpoVals( event, attr, 10*FW, TTOP+6*TFH+TVOFF/2, DR_DRSW2, 0, 0 ) ;
//	restoreEditColours() ;
...
////			else
////			{
//			drawIdxText( y*2+TVOFF/2, (char *)PSTR(STR_PROT_OPT), pModule->protocol, attr ) ;
////			}
//			if((y+=TFH)>(TLINES-1)*TFH+TTOP) return ;
//		}
//		subN += 1 ;
...
//			drawIdxText( y*2+TVOFF/2, (char *)XPSTR("\006  AutoSelect"), g_model.ForceTelemetryType, attr ) ;
//			if ( attr )
//			{
...
//				drawIdxText( y*2+TVOFF/2, (char *)XPSTR(M_NY_STR), value, attr ) ;
//				if(attr)
//				{
...
//				// Power stored in ppmNCH bit7 & Option stored in option_protocol
//				drawIdxText( y*2+TVOFF/2, (char *)XPSTR("\002 7 8 91011"), pModule->ppmFrameLength, attr ) ;
//				if(attr)
//				{
...
//					lcdDrawSolidFilledRectDMA( 0, y*TSCALE+2, TRIGHT*TSCALE, TFH*TSCALE-2, ~LcdBackground ) ;
//				}
//  			PUTS_ATT_COLOUR( THOFF, y+TVOFF/2, XPSTR("Failsafe"), 0, attr ? ~LcdForeground : LcdForeground ) ;
//				if ( ( PrivateData[1] && ( ( PrivateData[0] & 0x20 ) == 0 ) ) )
//				{
//	  			PUTS_ATT_COLOUR( 9*FW, y+TVOFF/2, XPSTR("N/A"), BLINK, attr ? ~LcdForeground : LcdForeground ) ;
//				}
//				else
//				{
//					if ( pModule->failsafeMode == 0 )
//					{
//	  				PUTS_ATT_COLOUR( 9*FW, y+TVOFF/2, XPSTR("(Not Set)"), 0, attr ? ~LcdForeground : LcdForeground ) ;
//					}
//					if ( sub == subN )
//					{
//						if ( checkForMenuEncoderLong( event ) )
...
//				drawIdxText( y*2+TVOFF/2, (char *)XPSTR(DSM2_STR), x, attr ) ;
...
//				drawIdxText( y*2, (char *)PSTR(STR_POS_NEG), pModule->pulsePol, attr ) ;
//				if ( attr )
//				{
...
//#endif
//				drawIdxText( y*2+TVOFF/2, (char *)XPSTR("\006D16(X)D8(D) LRP   R9M   "), pModule->sub_protocol, attr ) ;
//				if ( attr )
//				{
...
//				drawIdxText( y*2+TVOFF/2, (char *)XPSTR("\00216 8"), pModule->channels, attr ) ;
//				if ( attr )
//				{
...
//					drawIdxText( y*2+TVOFF/2, (char *)XPSTR("\006   OFF915MHz868MHz"), temp, attr ) ;
//					if ( attr )
//					{
...
//					drawIdxText( y*2+TVOFF/2, (char *)s, pModule->r9mPower, attr ) ;
//					if ( attr )
//					{
...
//				drawIdxText( y*2+TVOFF/2, (char *)XPSTR("\003AmeJapEur"), pModule->country, attr ) ;
//				if ( attr )
//				{
...
//					lcdDrawSolidFilledRectDMA( 0, y*TSCALE+2, TRIGHT*TSCALE, TFH*TSCALE-2, ~LcdBackground ) ;
//				}
//  			PUTS_ATT_COLOUR( THOFF, y+TVOFF/2, PSTR(STR_BIND), 0, attr ? ~LcdForeground : LcdForeground ) ;
//				if ( (pModule->protocol == PROTO_MULTI ) && ( PrivateData[1] ) )
//				{
//					div_t qr ;
//					saveEditColours( attr, DimBackColour ) ;
//					PUTS_ATT_LEFT( y+TVOFF/2, XPSTR("\006(Ver  . .  .  )" ) ) ;
//					PUTC( 11*FW, y+TVOFF/2, PrivateData[1] + '0' ) ;
//					PUTC( 13*FW, y+TVOFF/2, PrivateData[2] + '0' ) ;
//					qr = div( PrivateData[3], 10 ) ;
//					PUTC( 16*FW, y+TVOFF/2, qr.rem + '0' ) ;
//					if ( qr.quot )
//					{
//						PUTC( 15*FW, y+TVOFF/2, qr.quot + '0' ) ;
//					}
//					qr = div( PrivateData[4], 10 ) ;
//					PUTC( 19*FW, y+TVOFF/2, qr.rem + '0' ) ;
//					if ( qr.quot )
//					{
//						PUTC( 18*FW, y+TVOFF/2, qr.quot + '0' ) ;
//					}
//					if ( PrivateData[0] & 0x08 )
//					{
//						lcd_char_inverse( 0*FW, y+TVOFF/2, 4*FW, 1 ) ;
//					}
//					restoreEditColours() ;
//				}
//		  	if ( ( pModule->protocol == PROTO_DSM2) && ( pModule->sub_protocol == DSM_9XR ) )
...
//					// Display mode
//					saveEditColours( attr, DimBackColour ) ;
//					PUTS_ATT_LEFT( y+TVOFF/2, XPSTR("\006(Mode  )" ) ) ;
...
//					PUTS_NUM(  14*FW, y+TVOFF/2, XjtVersion, 0 ) ;
//					restoreEditColours() ;
//				}
//  		  if(attr)
//				{
...
//					lcdDrawSolidFilledRectDMA( 0, y*TSCALE+2, TRIGHT*TSCALE, TFH*TSCALE-2, ~LcdBackground ) ;
//				}
//  			PUTS_ATT_COLOUR( THOFF, y+TVOFF/2, PSTR(STR_RANGE), 0, attr ? ~LcdForeground : LcdForeground ) ;
//				if ( (pModule->protocol == PROTO_MULTI ) && ( PrivateData[1] ) )
//				{
...
//					{
//						saveEditColours( attr, DimBackColour ) ;
//						PUTS_P( TRIGHT-TRMARGIN-7*FW, y+TVOFF/2, XPSTR("Lqi") ) ;
//						restoreEditColours() ;
...
//				saveEditColours( attr, DimBackColour ) ;
//				putsChnOpRaw( TRIGHT-TRMARGIN-4*FW, y+TVOFF/2, value, 0, 0, 0 ) ;
//				restoreEditColours() ;
...
//						value += 1000 ;
//					}
//					value = editVarCapableValue( TRIGHT-TRMARGIN, y+TVOFF/2, value, -100, 100, NUM_VARS, attr, event ) ;
//					if ( value > 900 )
//					{
//						value -= 1000 ;
...
//				{
//#endif
//					pinput->weight = gvarMenuItem( TRIGHT-TRMARGIN, y+TVOFF/2, pinput->weight, -100, 100, GVAR_100|attr, event ) ;
//				}
//				restoreEditColours() ;
...
//				drawItem( (char *)PSTR(STR_2SWITCH)+1, y, attr ) ;
//				saveEditColours( attr, DimBackColour ) ;
//				putsDrSwitches(TRIGHT-TRMARGIN-4*FW, y+TVOFF/2, pinput->swtch, 0) ;
//				restoreEditColours() ;
...
//					if( attr && ( g_posHorz == p ) )
//					{
//						lcd_rectColour( TRIGHT-TRMARGIN-7*(FW+3)+p*(FW+2)-1, y+TVOFF/2-1, FW+2, 9, DimBackColour ) ;
...
//#endif
//				{
//					value = gvarMenuItem( TRIGHT-TRMARGIN, y+TVOFF/2, value, -100, 100, GVAR_100|attr, event ) ;
//				}
//				pinput->offset = value ;
//				restoreEditColours() ;
...
//    PUTS_NUM_N(  3*FW, y+TVOFF/2, k+1, LEADING0,2 ) ; //, fcolour) ;
//    if(k==g_eeGeneral.currModel)
//		{
////			lcd_putcAttColour(1, y+TVOFF, '*', 0, fcolour ) ;
//  		lcdDrawChar( (1)*2, (y+TVOFF)*2, '*', 0, fcolour ) ;
//		}
//    PUTS_ATT_N_COLOUR( 4*FW, y+TVOFF/2, (char *)ModelNames[k+1], sizeof(g_model.name), 0, fcolour ) ;
