...
#include "fdiskio.h"		/* Declarations of low level disk I/O functions */
...
//			return FR_DISK_ERR;
		...
		if (res == FR_OK) {			/* Fill sector window with new data */
			if (fdisk_read(fs->drv, fs->win, sector, 1) != RES_OK) {
...
{
	if (fdisk_read(fs->drv, fs->win, sect, 1) != RES_OK)	/* Load boot record */
		return 3;
...
)
{
	...
	*rfs = fs;							/* Return pointer to the corresponding file system object */
	...
	stat = fdisk_initialize(fs->drv);	/* Initialize the physical drive */
	...
	if (fmt == FS_FAT32) {
		...
		if (fdisk_read(fs->drv, fs->win, fs->fsi_sector, 1) == RES_OK &&
	...
	for ( ;  btr;								/* Repeat until all data read */
		...
		if ((fp->fptr % SS(fp->fs)) == 0) {		/* On the sector boundary? */
			...
			if (cc) {							/* Read maximum contiguous sectors directly */
				...
				if (fdisk_read(fp->fs->drv, rbuff, sect, (BYTE)cc) != RES_OK)
...
#endif
				if (fdisk_read(fp->fs->drv, fp->buf, sect, 1) != RES_OK)	/* Fill sector cache */
					ABORT(fp->fs, FR_DISK_ERR);
			}
#endif
			fp->dsect = sect;
	...
	for ( ;  btw;							/* Repeat until all data written */
		...
		if ((fp->fptr % SS(fp->fs)) == 0) {	/* On the sector boundary? */
			...
			if (fp->dsect != sect) {		/* Fill sector cache with file data */
				if (fp->fptr < fp->fsize &&
					fdisk_read(fp->fs->drv, fp->buf, sect, 1) != RES_OK)
						ABORT(fp->fs, FR_DISK_ERR);
			}
#endif
			fp->dsect = sect;
...
//					}
//#endif
//					if (fdisk_read(fp->fs->drv, fp->buf, dsc, 1) != RES_OK)	/* Load current sector */
//						ABORT(fp->fs, FR_DISK_ERR);
//#endif
//					fp->dsect = dsc;
//				}
...
#endif
			if (fdisk_read(fp->fs->drv, fp->buf, nsect, 1) != RES_OK)	/* Fill sector cache */
				ABORT(fp->fs, FR_DISK_ERR);
#endif
			fp->dsect = nsect;
		}
...
)
{
	...
	if (_MULTI_PARTITION && part) {
		...
		if (fdisk_read(pdrv, fs->win, 0, 1) != RES_OK) return FR_DISK_ERR;
