...
void WizMix::maxMixSwitch(char *name, SKYMixData &mix, int channel, int sw, int weight)
{
...
void WizMix::addMix(SKYModelData &model, Input input, int weight, int channel, int & mixIndex)
{
  if (input != NO_INPUT)  {
//    bool isTaranis = IS_TARANIS(GetEepromInterface()->getBoard());
    ...
    if (input >= RUDDER_INPUT && input <= AILERONS_INPUT) {
      SKYMixData & mix = model.mixData[mixIndex++];
...
//  model.name[WIZ_MODEL_NAME_LENGTH] = 0;
  ...
  if( options[THROTTLE_CUT_OPTION] && throttleChannel >=0 )
	{
    SKYSafetySwData *sd = &model.safetySw[throttleChannel] ;
		sd->opt.ss.mode = 3 ;
