...
void LogsDialog::on_fileOpen_BT_clicked()
{
  ...
  QString fileName = QFileDialog::getOpenFileName(this, tr("Select your log file"), settings.value("logdir").toString());//g.logDir());
  if (!fileName.isEmpty())
	{
    settings.setValue("logdir",QFileInfo(fileName).dir().absolutePath());
//    g.logDir(fileName);
...
bool LogsDialog::cvsFileParse()
{
  ...
  if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) { // reading HEX TEXT file
    ...
    if (buffer.startsWith("Date,Time"))
      ...
      file.reset();
		...
		{
	    if (buffer.startsWith("Time,Elapsed"))
			{
      	file.reset();
				ErskyFormat = 1 ;
			}
  ...
  file.close();
  if (errors > 1) {
    QMessageBox::warning(this, "Eepskye"/*CPN_STR_APP_NAME*/, tr("The selected logfile contains %1 invalid lines out of  %2 total lines").arg(errors).arg(lines));
  }
...
QDateTime LogsDialog::getRecordTimeStamp(int index)
{
  QString tstamp ;
	if (ErskyFormat)
	{
    tstamp = "2020-11-01 " + csvlog.at(index).at(0);
  if (csvlog.at(index).at(0).contains("."))
    return QDateTime::fromString(tstamp, "yyyy-MM-dd HH:mm:ss.zzz");
  return QDateTime::fromString(tstamp, "yyyy-MM-dd HH:mm:ss");
	}
	else
	{
  	tstamp = csvlog.at(index).at(0) + " " + csvlog.at(index).at(1);
    ...
    for (int row = 0; row < rowCount; row++) {
      ...
      if (hasLogSelection)
        ...
        logValue = ui->logTable->item(selectedRows.at(row), plotColumn);
				...
				{
	        time_str = "2020-11-01 " +
  	        ui->logTable->item(selectedRows.at(row), 0)->text();
				}
				else
				{
