...
class WizMix
{
  public:
    ...
    operator SKYModelData();

  private:
    WizMix();
    void addMix(SKYModelData & model, Input input, int weight, int channel, int & mixerIndex);
    void maxMixSwitch(char *name, SKYMixData &mix, int destCh, int sw, int weight);
		int stickMode ;
};
