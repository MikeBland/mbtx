...
{
//  return wizDlg->settings.settings.templateSetup;
	int x ;
	x = wizDlg->settings.templateSetup ;
	x *= 4 ;
	x += input-1 ;
	x = chout_ar[x] ;
//	x = (chout_ar[wizDlg->settings.templateSetup*4 + (input)-1]) ;
  return x-1 ;
}
  ...
  StandardPage(Page_Options, dlg, image, title, text, nextPage)
{
  ...
  l->addWidget(flightTimerRB);
	throttleTimerRB->setDisabled(true);
	flightTimerRB->setDisabled(true);
}
